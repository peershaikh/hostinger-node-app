...
exports.getPrimaryStation = exports.getStations = void 0;
 ...
 */
const DDR_ALIAS_CODES = ['CSMT', 'DR', 'DDR'];
const isDdrAliasEnabled = () => process.env.ENABLE_DDR_ALIAS === 'true';
/**
 ...
 */
const getStations = (input, mapping) => {
    ...
    cacheService_1.cacheService.set(cacheKey, uniqueResults, 3600);
    logger_1.winstonLogger.debug(`[STATION_RESOLVE] "${input}" → ${uniqueResults.join(', ')}`);
 ...
 */
const getPrimaryStation = (input, mapping) => {
    const stations = (0, exports.getStations)(input, mapping);
    return stations[0] || input.toUpperCase().trim();
};
exports.getPrimaryStation = getPrimaryStation;
