...
import { MEMORY_ALARMS, StationAlarmRecord } from '../controllers/alarmController';
...
function getAdaptiveIntervalMs(minDistanceKm: number | null): number {
  if (minDistanceKm === null) return 10 * 60 * 1000; // 10 min — no distance known yet
  if (minDistanceKm <= 25) return 30 * 1000;           // 30 sec — critical proximity
  if (minDistanceKm <= 50) return 2 * 60 * 1000;      // 2 min  — approaching
  if (minDistanceKm <= 100) return 5 * 60 * 1000;     // 5 min  — moderate distance
...
export class AlarmWorker {
  ...
  private trainDistanceCache = new Map<string, number>();

  start() {
    if (this.timer) return;
    winstonLogger.info('⏰ Alarm Background Worker: Starting background engine');
    this.run();
  }
  ...
  stop() {
    if (this.timer) {
      ...
      this.timer = null;
      winstonLogger.info('🛑 Alarm Worker stopped');
    }
  }
  ...
  private async run() {
    ...
    try {
      ...
      winstonLogger.error(`[ALARM_WORKER] Error in alarm cycle: ${err.message}`);
    } finally {
      ...
      winstonLogger.debug(
        `[ALARM_WORKER] Next cycle in ${nextLabel}` +
  ...
  private computeMinDistance(): number | null {
    if (this.trainDistanceCache.size === 0) return null;
    let min = Infinity;
  ...
  private async processAlarms() {
    ...
    try {
      ...
      if (!error && data) {
        ...
        const trainSet = new Set<string>(data.map((row: any) => row.train_no as string));
      ...
      winstonLogger.debug(`[ALARM_WORKER] DB fetch failed (table may be missing). Falling back to memory: ${dbErr.message}`);
    }

    // ── STEP 2: Merge memory-only trains (active during migration window) ────
    const memoryActive = Array.from(MEMORY_ALARMS.values()).filter(a => a.enabled);
    ...
    winstonLogger.info(`[ALARM_WORKER] Processing ${activeTrainNumbers.length} unique active train(s).`);
    ...
    for (const trainNo of activeTrainNumbers) {
      try {
        ...
        let trainAlarms: StationAlarmRecord[] = [];

        if (dbAvailable) {
          const { data: dbAlarms, error: alarmErr } = await supabase
            .from('user_station_alarms')
            .select('*')
            .eq('train_no', trainNo)
            .eq('enabled', true);
          ...
          if (!alarmErr && dbAlarms) {
            trainAlarms = dbAlarms as StationAlarmRecord[];
          }
        }
        ...
        let nearestDistanceForTrain: number | null = null;
        ...
        winstonLogger.error(`[ALARM_WORKER] Failed to process train ${trainNo}: ${trainErr.message}`);
      }
    }
   ...
   */
  private async evaluateAlarmProximity(alarm: StationAlarmRecord, status: any): Promise<number | null> {
    try {
      const destCoords = await stationService.getCoordinates(alarm.destination_station);
      if (!destCoords) {
        winstonLogger.warn(`[ALARM_WORKER] Cannot find coordinates for target station: ${alarm.destination_station}`);
        return null;
      }
      ...
      if (status.latitude && status.longitude) {
        ...
        winstonLogger.debug(
          `[ALARM_WORKER] GPS check: Train ${alarm.train_no} is ${distanceKm.toFixed(2)} km from ${alarm.destination_station}`
        );
      }
      // Method B: Current Station Proximity (Fallback when GPS coords absent)
      else if (status.current_station) {
        const currentStationCode =
          status.journey_timeline?.[status.current_station_index || 0]?.station_code ||
          status.current_station;
        const currentCoords = await stationService.getCoordinates(currentStationCode);
        if (currentCoords) {
          distanceKm = this.calculateHaversine(
            currentCoords.lat,
            currentCoords.lon,
      ...
      if (distanceKm !== null && distanceKm <= alarm.radius_km) {
        winstonLogger.info(
          `[ALARM_WORKER] 🚨 Alarm TRIGGERED: Train ${alarm.train_no} is within ${distanceKm.toFixed(1)}km of ${alarm.destination_station}`
        );
        ...
        if (!disabled) {
          ...
          winstonLogger.warn(`[ALARM_WORKER] Alarm ${alarm.id} already disabled — skipping duplicate dispatch`);
          return distanceKm;
        }

        // Queue alert via the existing notification pipeline
        await alertService.queueAlert({
          userId: alarm.user_id || alarm.device_id,
          type: 'WAKEUP_ALARM' as any,
          priority: 'HIGH',
          metadata: {
      ...
      winstonLogger.error(`[ALARM_WORKER] Failed to evaluate alarm ${alarm.id}: ${err.message}`);
      return null;
    }
  }
   ...
   */
  private async atomicDisableAlarm(alarmId: string): Promise<boolean> {
    try {
      ...
      if (data && data.length > 0) {
        winstonLogger.info(`[ALARM_WORKER] Atomically disabled alarm ${alarmId} in Supabase`);
        return true;
      }
      ...
      winstonLogger.debug(`[ALARM_WORKER] Supabase atomic disable failed, disabling in memory: ${e.message}`);

      const mem = MEMORY_ALARMS.get(alarmId);
      if (mem && mem.enabled) {
        MEMORY_ALARMS.set(alarmId, {
          ...mem,
          enabled: false,
          updated_at: new Date().toISOString()
        });
        winstonLogger.info(`[ALARM_WORKER] Disabled alarm ${alarmId} in memory`);
        return true;
      }
...
export const alarmWorker = new AlarmWorker();
