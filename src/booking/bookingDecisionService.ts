...
import { BookingIntent, BookingProvider, ProviderId } from './interfaces';
import { bookingProviderRegistry } from './providerRegistry';
import {
  DecisionContext,
  DecisionReason,
  DecisionResult,
...
function capabilityForMode(mode: JourneyMode): keyof typeof CAPABILITY_MAP {
  return mode;
}

const CAPABILITY_MAP = {
  RAIL:   (p: BookingProvider) => p.capabilities.supportsRail,
  BUS:    (p: BookingProvider) => p.capabilities.supportsBus,
  FLIGHT: (p: BookingProvider) => p.capabilities.supportsFlight,
  HOTEL:  (p: BookingProvider) => p.capabilities.supportsHotel
} as const;
...
class BookingDecisionService {
   ...
   */
  evaluate(context: DecisionContext, _providers?: ReadonlyArray<BookingProvider>): DecisionResult {
    const evaluatedAt = new Date().toISOString();

    // Feature flag gate: engine disabled
    if (!featureFlags.bookingDecision) {
      return {
    ...
    try {
      const allProviders = _providers ?? bookingProviderRegistry.getAll();
      const allScores: DecisionScore[] = allProviders.map(p => scoreProvider(p, context));
  ...
  buildContext(
    ...
    return {
      ...
      isPremiumMode:   opts.isPremiumMode ?? featureFlags.directBooking,
      isPartnerRouter: featureFlags.partnerRouter,
...
export const bookingDecisionService = new BookingDecisionService();
