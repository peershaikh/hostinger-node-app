...
class UserRepository {
  ...
  async update(id: string, updates: Partial<User>): Promise<User> {
    ...
    try {
      ...
      // data is null when the user exists only in local file store (not yet in Supabase)
      if (!data) return { id, ...updates } as any;
      return this.mapToApp(data);
    } catch (err: any) {
      if (err instanceof DatabaseError) throw err;
  ...
  async incrementUsage(id: string, feature: 'searches' | 'pnr' | 'live'): Promise<void> {
    if (!isSupabaseConfigured()) {
      throw new DatabaseError('NOT_CONFIGURED', 'Supabase not configured');
    }
    ...
    try {
      let column = '';
      if (feature === 'searches') column = 'daily_search_count';
      else if (feature === 'pnr') column = 'daily_pnr_count';
      else if (feature === 'live') column = 'daily_live_count';
      
      const { data: user, error: fetchErr } = await supabase.from('users').select(column).eq('id', id).single();
      if (fetchErr) throw new DatabaseError(fetchErr.code, fetchErr.message);

      const payload = { [column]: ((user as any)[column] || 0) + 1 };
      const { error: updateErr } = await supabase.from('users').update(payload).eq('id', id);
      if (updateErr) throw new DatabaseError(updateErr.code, updateErr.message);
    } catch (err: any) {
      if (err instanceof DatabaseError) throw err;
  ...
  async updateTokenVersion(id: string, newVersion: number): Promise<void> {
    if (!isSupabaseConfigured()) {
      throw new DatabaseError('NOT_CONFIGURED', 'Supabase not configured');
    }
    if (!id || !UUID_REGEX.test(id)) return;
    try {
      const { error } = await supabase.from('users').update({ token_version: newVersion }).eq('id', id);
      if (error) throw new DatabaseError(error.code, error.message);
    } catch (err: any) {
      if (err instanceof DatabaseError) throw err;
  ...
  async resetAllLimits(): Promise<void> {
    ...
    try {
      ...
      const { error } = await supabase.from('users').update(payload).neq('id', 'placeholder-avoid-full-table-error-if-needed');
      if (error) throw new DatabaseError(error.code, error.message);
    } catch (err: any) {
      if (err instanceof DatabaseError) throw err;
  ...
  async batchUpsert(users: User[]): Promise<void> {
    if (!isSupabaseConfigured()) {
      throw new DatabaseError('NOT_CONFIGURED', 'Supabase not configured');
    }
    try {
