...
export class TrainController {
  ...
  searchAdvanced = async (req: Request, res: Response) => {
    try {
      ...
      if (includeSplit && finalSplits.length === 0) {
        setImmediate(() => {
          const diag = (splitResult as any)?.diagnostic || {};
          ...
          selfLearningService.logMissingRoute(source, destination, userId, {
  ...
  getAvailability = async (req: Request, res: Response) => {
    try {
      ...
      const buildEntry = (raw: any): {
        ...
        let quotaType: 'GNWL' | 'RLWL' | 'PQWL' | 'TQWL' | 'RSWL' | 'GENERAL' | undefined;
        let formattedStatus = textStatus || rawStatus;
        let wlNum: number | null = null;
        ...
        if (isWl) {
          ...
          if (currentWlMatch && currentWlMatch[1]) {
            ...
            if (anyDigits) wlNum = parseInt(anyDigits[0], 10);
          }
   ...
   */
  getDailyCancellations = async (req: Request, res: Response) => {
    try {
      const today = new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' });
