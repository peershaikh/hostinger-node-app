...
export interface LearningStats {
  date: string;
  new_trains_count: number;
  new_schedules_count: number;
  api_hits_saved: number;
  updated_at?: string;
}
...
export interface ProviderHealthDetail {
  status: 'ONLINE' | 'DEGRADED' | 'CRITICAL' | 'UNKNOWN';
...
export interface SystemStatusReport {
  ...
  today_trains_learned: number;
  today_schedules_learned: number;
  growth_percentage: string;
  api_hits_saved_today: number;
  ...
  system_health: 'OPTIMAL' | 'DEGRADED' | 'ERROR';
  providers: {
...
export class MetricsService {
  private readonly LEARNING_STATS_TABLE = 'learning_stats';
  private readonly TRAINS_TABLE = 'trains';
  private readonly SCHEDULES_TABLE = 'train_schedule';

  private readonly metricsWindow: RequestMetric[] = [];
  private readonly WINDOW_LIMIT = 100;
  ...
  startSnapshotScheduler(): void {
    if (process.env.ENABLE_PERSISTENT_METRICS !== 'true') {
      winstonLogger.info('[METRICS] Persistent snapshots disabled by feature flag.');
      return;
    }

    if (this.snapshotInterval) {
      winstonLogger.warn('[METRICS] Snapshot scheduler already running.');
      return;
    }

    winstonLogger.info('[METRICS] Starting background system metrics snapshot scheduler (5-minute interval).');
    ...
    this.snapshotInterval = setInterval(async () => {
      try {
        ...
        Promise.resolve(
          ...
          .then(({ error }) => {
            if (error) {
              winstonLogger.warn(`[METRICS_SNAPSHOT_ERROR] Failed to save snapshot: ${error.message}`);
            } else {
              winstonLogger.debug('[METRICS_SNAPSHOT] Dynamic metrics snapshot committed to database.');
            }
          })
          .catch((err: any) => {
            winstonLogger.error(`[METRICS_SNAPSHOT_EXCEPTION] ${err.message}`);
          });

      } catch (err: any) {
        winstonLogger.error(`[METRICS_SNAPSHOT_SCHEDULER_ERR] ${err.message}`);
      }
    }, 5 * 60 * 1000);
  }
  ...
  stopSnapshotScheduler(): void {
    if (this.snapshotInterval) {
      ...
      winstonLogger.info('[METRICS] Stopped system metrics snapshot scheduler.');
    }
  }
   ...
   */
  recordProviderRequest(provider: string, latency: number, success: boolean): void {
    const matched = Object.keys(this.providerWindows).find(
    ...
    if (!success) {
      ...
      if (this.consecutiveFailures[matched] === 3) {
        winstonLogger.warn(`[CIRCUIT_BREAKER_TRIGGERED] Provider "${matched}" has registered 3 consecutive failures. Opening circuit breaker.`);
        this.triggerCircuitBreaker(matched).catch(err => {
          winstonLogger.error(`[CIRCUIT_BREAKER_DB_ERR] Failed to execute circuit breaker DB toggle: ${err.message}`);
        });
      }
    } else {
   ...
   */
  private async triggerCircuitBreaker(provider: string): Promise<void> {
  ...
  async trackLearning(
    ...
    try {
      if (trains === 0 && schedules === 0 && cacheHits === 0) return true;
      ...
      if (trains > 0) {
        promises.push(
          Promise.resolve(
            supabase.rpc('increment_learning_stats', {
      ...
      winstonLogger.debug(`[METRICS] Tracked learning → Trains: ${trains}, Schedules: ${schedules}, Cache saved: ${cacheHits}`);
      return true;
    } catch (e: any) {
      winstonLogger.warn(`[METRICS_FAIL] trackLearning failed: ${e.message}`);
      return false;
    }
  }
  ...
  async getSystemStatus(): Promise<SystemStatusReport> {
    ...
    try {
      ...
      try {
        const { count: tCount, error: tErr } = await supabase.from(this.TRAINS_TABLE).select('*', { count: 'exact', head: true });
        if (!tErr && tCount !== null && tCount !== undefined) {
          totalTrains = tCount;
        } else {
          const { count: regCount } = await supabase.from('train_registry').select('*', { count: 'exact', head: true });
          totalTrains = regCount || 0;
        }
      } catch (trainsErr: any) {
        winstonLogger.warn(`[METRICS] Trains count query fallback: ${trainsErr.message}`);
      }

      try {
        const { count: sCount } = await supabase.from(this.SCHEDULES_TABLE).select('*', { count: 'exact', head: true });
      ...
      let todayStatsResult: any = { data: [] };
      let yesterdayStatsResult: any = { data: [] };
      try {
        ...
        winstonLogger.warn(`[METRICS] Learning stats query fallback: ${learningErr.message}`);
      }

      const todayMap: Record<string, number> = {};
      const yesterdayMap: Record<string, number> = {};

      (todayStatsResult.data || []).forEach((row: any) => {
        const metric = row.stat_key?.split(':')[0] || '';
        todayMap[metric] = Number(row.stat_value || 0);
      });

      (yesterdayStatsResult.data || []).forEach((row: any) => {
        const metric = row.stat_key?.split(':')[0] || '';
      ...
      if (totalSamples > 0) {
        ...
        try {
          ...
          winstonLogger.warn(`[METRICS_SNAPSHOT_QUERY_FAIL] ${snapshotErr.message}`);
        }
      }
      ...
      const cpu_load = parseFloat(os.loadavg()[0].toFixed(2));

      // Calculate health: degraded if error rate > 0% but <= 5%, error if > 5%
      let dynamicHealth: 'OPTIMAL' | 'DEGRADED' | 'ERROR' = 'OPTIMAL';
      if (errorRate > 5) {
        dynamicHealth = 'ERROR';
      } else if (errorRate > 0) {
      ...
      const getProviderDetail = (provider: string): ProviderHealthDetail => {
        ...
        let pStatus: 'ONLINE' | 'DEGRADED' | 'CRITICAL' | 'UNKNOWN' = 'ONLINE';
        if (consecFail >= 3) {
          pStatus = 'CRITICAL';
      ...
      winstonLogger.debug(`[METRICS] System status generated | Trains: ${report.total_trains} | Today learned: ${report.today_trains_learned}`);
      return report;
    } catch (e: any) {
      winstonLogger.error(`[METRICS] getSystemStatus failed: ${e.message}`);
      ...
      return {
        ...
        heap_usage_mb: 0,
        cpu_load: 0,
        logical_cpu_count: os.cpus().length || 1,
        system_health: "ERROR",
        providers: {
   ...
   */
  async getLearningHistory(days: number = 7): Promise<LearningStats[]> {
    try {
      ...
      const dateMap: Record<string, LearningStats> = {};
      ...
      winstonLogger.warn(`[METRICS] Failed to fetch learning history: ${e.message}`);
      return [];
    }
  }
   ...
   */
  async getSystemHistory(days: number = 7): Promise<any[]> {
    try {
      ...
      const dateMap: Record<string, {
        date: string;
        avg_latency_ms_sum: number;
        p95_latency_ms_max: number;
        error_rate_percent_sum: number;
        requests_count_sum: number;
        samples: number;
      }> = {};
      ...
      winstonLogger.warn(`[METRICS] Failed to fetch system performance history: ${e.message}`);
      return [];
    }
  }
}
