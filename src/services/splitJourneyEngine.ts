...
interface RichLeg extends Leg {
  fromCode: string;   // departure station code of this leg
  toCode: string;   // arrival station code of this leg
  fromName: string;
  toName: string;
  durationMins: number; // actual leg duration in minutes
  journeyDate?: string;
  travelDate?: string;
  depDay?: number;
  running_days?: string;
...
/** PHASE_4C970 PRO GATE — added isPremiumUser for paywall isolation */
export interface SplitRouteOptions {
  classType?: string;
  quota?: string;
  isPremiumUser?: boolean; // PHASE_4C970: if true, expose full premium output
}
 ...
 */
export function calculateWaitlistVelocity(
  trainNo: string,
  ...
  confProbability: number;
  daysToConfirm: number;
  isLocked: boolean;    // always false here; gate applied by applyPremiumGate()
} {
  ...
  const DROP_RATE_TABLE: Record<string, number> = {
  ...
  const confProbability =
    daysToConfirm <= 0  ? 95 :
    daysToConfirm <= 7  ? 82 :
    daysToConfirm <= 14 ? 58 : 28;
  const velocityLabel =
    confProbability >= 80 ? 'HIGH_VELOCITY' :
 ...
 */
export function exploreCrossClassMatrix(
  requestedClass: string,
  leg1Status: string,
  leg2Status: string,
  leg1DepartureHour: number,
  leg2DepartureHour: number
): {
  legClasses: string[];
  rationale: string;
} | null {
  const isRegret = (s: string) =>
    /REGRET|NOT AVAILABLE|CLASS NOT AVAILABLE|WL\s*[5-9][0-9]|WL\s*[1-9][0-9]{2}/i.test(s);

  if (!isRegret(leg1Status) && !isRegret(leg2Status)) return null;
 ...
 */
export function applyPremiumGate(
  splits: any[],
  isPremiumUser: boolean,
  requestedClass: string = 'SL',
  source: string = '',
  destination: string = ''
): any[] {
  ...
  return splits.map(s => {
    ...
    const sameTrainPremiumFields = isSameTrain && !isPremiumUser
      ? { paywallType: 'SAME_TRAIN_REUSE' as const, isLocked: true }
      : {};
    ...
    if (legs.length >= 2) {
      ...
      const ccamRaw = exploreCrossClassMatrix(
        requestedClass,
        leg1Status,
        leg2Status,
        parseHour(legs[0]?.departure || ''),
        parseHour(legs[1]?.departure || '')
      );
      if (ccamRaw) {
        ccam = isPremiumUser
...
const getCity = (code: string): string => {
  return stationService.getCitySync(code);
};
...
// —————————————————————————————————————————————————————————————————————————————
const PAN_INDIA_CORRIDOR_HUBS: Record<string, string[]> = {
...
const DETERMINISTIC_CORRIDORS: Record<string, string[]> = {
  ...
  "mumbai-guwahati":   ["HWH", "NJP", "NGP", "BSP", "ROU", "MLDT", "KGP", "BSL", "DDU", "PNBE", "KIR", "NBQ"],
  "guwahati-mumbai":   ["HWH", "NJP", "NGP", "BSP", "ROU", "MLDT", "KGP", "BSL", "DDU", "PNBE", "KIR", "NBQ"],
  "mumbai-ghy":        ["HWH", "NJP", "NGP", "BSP", "ROU", "MLDT", "KGP", "BSL", "DDU", "PNBE", "KIR", "NBQ"],
  "ghy-mumbai":        ["HWH", "NJP", "NGP", "BSP", "ROU", "MLDT", "KGP", "BSL", "DDU", "PNBE", "KIR", "NBQ"],
  "mumbai-kyq":        ["HWH", "NJP", "NGP", "BSP", "ROU", "MLDT", "KGP", "BSL", "DDU", "PNBE", "KIR", "NBQ"],
  "kyq-mumbai":        ["HWH", "NJP", "NGP", "BSP", "ROU", "MLDT", "KGP", "BSL", "DDU", "PNBE", "KIR", "NBQ"],
  "mumbai-ngc":        ["HWH", "NJP", "NGP", "BSP", "ROU", "MLDT", "KGP", "BSL", "DDU", "PNBE", "KIR", "NBQ"],
  "ngc-mumbai":        ["HWH", "NJP", "NGP", "BSP", "ROU", "MLDT", "KGP", "BSL", "DDU", "PNBE", "KIR", "NBQ"],
  "panvel-guwahati":   ["HWH", "NJP", "NGP", "BSP", "ROU", "MLDT", "KGP", "BSL", "DDU", "PNBE", "KIR", "NBQ"],
  "guwahati-panvel":   ["HWH", "NJP", "NGP", "BSP", "ROU", "MLDT", "KGP", "BSL", "DDU", "PNBE", "KIR", "NBQ"],
  ...
  "mumbai-amritsar":    ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "amritsar-mumbai":    ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "mumbai-asr":         ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "asr-mumbai":         ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "mumbai-jalandhar":   ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "jalandhar-mumbai":   ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "mumbai-juc":         ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "juc-mumbai":         ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "mumbai-beas":        ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "beas-mumbai":        ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "panvel-amritsar":    ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "amritsar-panvel":    ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "panvel-asr":         ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "asr-panvel":         ["NDLS", "LDH", "UMB", "KOTA", "RTM", "BRC", "MTJ", "BSL", "ET", "BPL", "JUC", "ST"],
  "mumbai-jammu":     ["NDLS", "UMB", "LDH", "KOTA", "RTM", "BRC", "ST", "MTJ", "JRC", "PTKC", "JAT", "BPL"],
  "jammu-mumbai":     ["NDLS", "UMB", "LDH", "KOTA", "RTM", "BRC", "ST", "MTJ", "JRC", "PTKC", "JAT", "BPL"],
  "mumbai-katra":     ["NDLS", "UMB", "LDH", "KOTA", "RTM", "BRC", "ST", "MTJ", "JRC", "PTKC", "JAT", "BPL"],
  "katra-mumbai":     ["NDLS", "UMB", "LDH", "KOTA", "RTM", "BRC", "ST", "MTJ", "JRC", "PTKC", "JAT", "BPL"],
  "mumbai-jat":       ["NDLS", "UMB", "LDH", "KOTA", "RTM", "BRC", "ST", "MTJ", "JRC", "PTKC", "JAT", "BPL"],
  "jat-mumbai":       ["NDLS", "UMB", "LDH", "KOTA", "RTM", "BRC", "ST", "MTJ", "JRC", "PTKC", "JAT", "BPL"],
  "mumbai-svdk":      ["NDLS", "UMB", "LDH", "KOTA", "RTM", "BRC", "ST", "MTJ", "JRC", "PTKC", "JAT", "BPL"],
  "svdk-mumbai":      ["NDLS", "UMB", "LDH", "KOTA", "RTM", "BRC", "ST", "MTJ", "JRC", "PTKC", "JAT", "BPL"],
  "panvel-jammu":     ["NDLS", "UMB", "LDH", "KOTA", "RTM", "BRC", "ST", "MTJ", "JRC", "PTKC", "JAT", "BPL"],
  ...
  "mumbai-kanpur":     ["CNB", "ET", "BPL", "JHS", "VGLJ", "BSL", "NDLS", "PRYJ", "LKO", "AGC"],
  "kanpur-mumbai":     ["CNB", "ET", "BPL", "JHS", "VGLJ", "BSL", "NDLS", "PRYJ", "LKO", "AGC"],
  "mumbai-agra":       ["AGC", "MTJ", "NDLS", "KOTA", "BPL", "ET", "JHS", "GWL", "BRC", "ST"],
  ...
  // —— Mumbai – Raipur ——
  "mumbai-raipur":     ["R", "NGP", "G", "DURG", "BSP", "BSL", "ET", "BPL"],
  "raipur-mumbai":     ["R", "NGP", "G", "DURG", "BSP", "BSL", "ET", "BPL"],

  // —— Mumbai – Gaya ——
  "mumbai-gaya":       ["GAYA", "DDU", "PNBE", "MGS", "ET", "BPL", "BSL", "JHS", "PRYJ", "NGP"],
  "gaya-mumbai":       ["GAYA", "DDU", "PNBE", "MGS", "ET", "BPL", "BSL", "JHS", "PRYJ", "NGP"],
  ...
  "bhopal-mumbai":   ["ET", "BSL", "KNW", "UJN", "RTM"],
  "mumbai-bpl":      ["ET", "BSL", "KNW", "UJN", "RTM", "MMR", "BRC", "ST", "NAD", "JL"],
  "bpl-mumbai":      ["ET", "BSL", "KNW", "UJN", "RTM"],
  "mumbai-rkmp":     ["ET", "BSL", "KNW", "UJN", "RTM", "MMR", "BRC", "ST", "NAD", "JL"],
  "rkmp-mumbai":     ["ET", "BSL", "KNW", "UJN", "RTM"],
  "mumbai-hbj":      ["ET", "BSL", "KNW", "UJN", "RTM", "MMR", "BRC", "ST", "NAD", "JL"],
  "hbj-mumbai":      ["ET", "BSL", "KNW", "UJN", "RTM"],
  "panvel-bhopal":   ["ET", "BSL", "KNW", "UJN", "RTM", "MMR", "BRC", "ST", "NAD", "JL"],
  "bhopal-panvel":   ["ET", "BSL", "KNW", "UJN", "RTM"],
  "panvel-bpl":      ["ET", "BSL", "KNW", "UJN", "RTM", "MMR", "BRC", "ST", "NAD", "JL"],
  "bpl-panvel":      ["ET", "BSL", "KNW", "UJN", "RTM"],
  ...
  // —— Delhi – Kanpur ——
  "delhi-kanpur": ["ALJN", "TDL", "ETW", "GZB", "FZD", "SKB", "HRS", "MTJ", "AGC", "PHD", "KRJ"],
  "kanpur-delhi": ["ALJN", "TDL", "ETW", "GZB", "FZD", "SKB", "HRS", "MTJ", "AGC", "PHD", "KRJ"],
  ...
  "bhopal-delhi": ["JHS", "VGLJ", "GWL", "AGC", "MTJ"],
  "delhi-jaipur": ["AGC", "KOTA", "JP"],
  "jaipur-delhi": ["AWR", "RE", "BKI", "AII"],
  ...
  // —— Delhi – Jabalpur ——
  "delhi-jabalpur": ["ET", "BPL", "JHS", "VGLJ", "KTE", "STA", "CNB", "BINA"],
  "jabalpur-delhi": ["ET", "BPL", "JHS", "VGLJ", "KTE", "STA", "CNB", "BINA"],
  ...
  "trivandrum-chennai": ["ERS", "CBE", "SA"],
  "chennai-delhi": ["BZA", "WL", "KZJ", "BPQ", "NGP", "ET", "BPL", "VGLJ", "GWL", "AGC", "MTJ", "SC"],
  ...
  "chennai-ranchi": ["BZA", "VSKP", "KGP", "HWH", "ROU", "TATA", "GAYA", "NGP"],
  "ranchi-chennai": ["BZA", "VSKP", "KGP", "HWH", "ROU", "TATA", "GAYA", "NGP"],
  "chennai-gaya": ["BZA", "VSKP", "KGP", "HWH", "PNBE", "DDU", "NGP"],
  ...
  "cnb-mas":          ["VGLJ", "ET", "NGP", "BZA", "BPL", "BPQ", "PRYJ", "WL", "KZJ", "BINA", "LKO", "SC"],
  "chennai-indore":   ["UJN", "BPL", "ET", "NGP", "BZA", "BPQ", "WL", "KZJ", "RTM", "SC", "GDR", "NAD"],
  ...
  "chennai-chandigarh": ["UMB", "NDLS", "PNP", "AGC", "VGLJ", "BPL", "ET", "NGP", "BPQ", "WL", "BZA", "SC"],
  "chandigarh-chennai": ["UMB", "NDLS", "PNP", "AGC", "VGLJ", "BPL", "ET", "NGP", "BPQ", "WL", "BZA", "SC"],
  "mas-cdg":            ["UMB", "NDLS", "PNP", "AGC", "VGLJ", "BPL", "ET", "NGP", "BPQ", "WL", "BZA", "SC"],
  "cdg-mas":            ["UMB", "NDLS", "PNP", "AGC", "VGLJ", "BPL", "ET", "NGP", "BPQ", "WL", "BZA", "SC"],
  "chennai-amritsar":   ["JUC", "LDH", "UMB", "NDLS", "VGLJ", "BPL", "ET", "NGP", "BPQ", "WL", "BZA", "SC"],
  "amritsar-chennai":   ["JUC", "LDH", "UMB", "NDLS", "VGLJ", "BPL", "ET", "NGP", "BPQ", "WL", "BZA", "SC"],
  "mas-asr":            ["JUC", "LDH", "UMB", "NDLS", "VGLJ", "BPL", "ET", "NGP", "BPQ", "WL", "BZA", "SC"],
  "asr-mas":            ["JUC", "LDH", "UMB", "NDLS", "VGLJ", "BPL", "ET", "NGP", "BPQ", "WL", "BZA", "SC"],
  "chennai-dehradun":   ["HW", "SRE", "NDLS", "AGC", "VGLJ", "BPL", "ET", "NGP", "BPQ", "WL", "BZA", "SC"],
  "dehradun-chennai":   ["HW", "SRE", "NDLS", "AGC", "VGLJ", "BPL", "ET", "NGP", "BPQ", "WL", "BZA", "SC"],
  ...
  // —— Bengaluru ↔ Delhi deterministic corridor ——
  "bengaluru-delhi": ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "bangalore-delhi": ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "bengaluru-ndls":  ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "ndls-bengaluru":  ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "bangalore-ndls":  ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "ndls-bangalore":  ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "bengaluru-nzm":   ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "nzm-bengaluru":   ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "bangalore-nzm":   ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "nzm-bangalore":   ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "bengaluru-dli":   ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "dli-bengaluru":   ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "bangalore-dli":   ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "dli-bangalore":   ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "bengaluru-anvt":  ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "anvt-bengaluru":  ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "bangalore-anvt":  ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "anvt-bangalore":  ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "sbc-delhi":       ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "delhi-sbc":       ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "ypr-delhi":       ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "delhi-ypr":       ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "smvb-delhi":      ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "delhi-smvb":      ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "sbc-ndls":        ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "ndls-sbc":        ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "ypr-ndls":        ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "ndls-ypr":        ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "smvb-ndls":       ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "ndls-smvb":       ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "sbc-nzm":         ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "nzm-sbc":         ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "ypr-nzm":         ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "nzm-ypr":         ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "smvb-nzm":        ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "nzm-smvb":        ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "sbc-dli":         ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "dli-sbc":         ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "ypr-dli":         ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "dli-ypr":         ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "smvb-dli":        ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "dli-smvb":        ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "sbc-anvt":        ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "anvt-sbc":        ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "ypr-anvt":        ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "anvt-ypr":        ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "smvb-anvt":       ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  "anvt-smvb":       ["AGC", "VGLJ", "BPL", "ET", "NGP", "GWL", "BPQ", "SC", "GTL", "WADI", "MTJ", "BZA"],
  // —— Bengaluru ↔ Kolkata deterministic corridor ——
  "bengaluru-kolkata": ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "kolkata-bengaluru": ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "bangalore-kolkata": ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "kolkata-bangalore": ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "bengaluru-howrah":  ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "howrah-bengaluru":  ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "bangalore-howrah":  ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "howrah-bangalore":  ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "sbc-hwh":           ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "hwh-sbc":           ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "ypr-hwh":           ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "hwh-ypr":           ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "smvb-hwh":          ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "hwh-smvb":          ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "sbc-sdah":          ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "sdah-sbc":          ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "ypr-sdah":          ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "sdah-ypr":          ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "smvb-sdah":         ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "sdah-smvb":         ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "sbc-koaa":          ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "koaa-sbc":          ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "ypr-koaa":          ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "koaa-ypr":          ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "smvb-koaa":         ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "koaa-smvb":         ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "sbc-shm":           ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "shm-sbc":           ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "ypr-shm":           ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "shm-ypr":           ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "smvb-shm":          ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "shm-smvb":          ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "sbc-kolkata":       ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "kolkata-sbc":       ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "ypr-kolkata":       ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "kolkata-ypr":       ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "smvb-kolkata":      ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "kolkata-smvb":      ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "sbc-howrah":        ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "howrah-sbc":        ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "ypr-howrah":        ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "howrah-ypr":        ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "smvb-howrah":       ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "howrah-smvb":       ["BZA", "VSKP", "KGP", "BBS", "CTC", "RU", "BLS", "RJY", "NGP", "SC", "TATA", "KPD"],
  "bengaluru-chennai": ["KPD", "JTJ", "BWT"],
  "bangalore-chennai": ["KPD", "JTJ", "BWT"],
  // —— Bengaluru ↔ Patna deterministic corridor ——
  "bengaluru-patna": ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "patna-bengaluru": ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "bangalore-patna": ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "patna-bangalore": ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "bengaluru-pnbe":  ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "pnbe-bengaluru":  ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "bangalore-pnbe":  ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "pnbe-bangalore":  ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "bengaluru-dnr":   ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "dnr-bengaluru":   ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "bangalore-dnr":   ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "dnr-bangalore":   ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "sbc-pnbe":        ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "pnbe-sbc":        ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "ypr-pnbe":        ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "pnbe-ypr":        ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "smvb-pnbe":       ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "pnbe-smvb":       ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "sbc-dnr":         ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "dnr-sbc":         ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "ypr-dnr":         ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "dnr-ypr":         ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "smvb-dnr":        ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  "dnr-smvb":        ["DDU", "JBP", "ET", "NGP", "STA", "PCOI", "BZA", "SC", "BPQ", "GTL", "WADI", "GAYA"],
  // —— Bengaluru – Secunderabad / Hyderabad ——
  "bengaluru-secunderabad": ["GTL", "DMM", "DHNE", "KRNT", "MBNR", "RC", "WADI", "VKB", "ATP", "YG", "TDU", "HUP"],
  "secunderabad-bengaluru": ["GTL", "DMM", "DHNE", "KRNT", "MBNR", "RC", "WADI", "VKB", "ATP", "YG", "TDU", "HUP"],
  "bengaluru-hyderabad":    ["GTL", "DMM", "DHNE", "KRNT", "MBNR", "RC", "WADI", "VKB", "ATP", "YG", "TDU", "HUP"],
  "hyderabad-bengaluru":    ["GTL", "DMM", "DHNE", "KRNT", "MBNR", "RC", "WADI", "VKB", "ATP", "YG", "TDU", "HUP"],
  "bangalore-secunderabad": ["GTL", "DMM", "DHNE", "KRNT", "MBNR", "RC", "WADI", "VKB", "ATP", "YG", "TDU", "HUP"],
  "secunderabad-bangalore": ["GTL", "DMM", "DHNE", "KRNT", "MBNR", "RC", "WADI", "VKB", "ATP", "YG", "TDU", "HUP"],
  "bangalore-hyderabad":    ["GTL", "DMM", "DHNE", "KRNT", "MBNR", "RC", "WADI", "VKB", "ATP", "YG", "TDU", "HUP"],
  "hyderabad-bangalore":    ["GTL", "DMM", "DHNE", "KRNT", "MBNR", "RC", "WADI", "VKB", "ATP", "YG", "TDU", "HUP"],
  "bengaluru-sc":           ["GTL", "DMM", "DHNE", "KRNT", "MBNR", "RC", "WADI", "VKB", "ATP", "YG", "TDU", "HUP"],
  "sc-bengaluru":           ["GTL", "DMM", "DHNE", "KRNT", "MBNR", "RC", "WADI", "VKB", "ATP", "YG", "TDU", "HUP"],
  "bengaluru-kcg":          ["GTL", "DMM", "DHNE", "KRNT", "MBNR", "RC", "WADI", "VKB", "ATP", "YG", "TDU", "HUP"],
  ...
  // —— Bengaluru ↔ Lucknow deterministic corridor ——
  "bengaluru-lucknow": ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "lucknow-bengaluru": ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "bangalore-lucknow": ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "lucknow-bangalore": ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "bengaluru-lko":      ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "lko-bengaluru":      ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "bangalore-lko":      ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "lko-bangalore":      ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "bengaluru-ljn":      ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "ljn-bengaluru":      ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "bangalore-ljn":      ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "ljn-bangalore":      ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "sbc-lko":            ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "lko-sbc":            ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "ypr-lko":            ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "lko-ypr":            ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "smvb-lko":           ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "lko-smvb":           ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "sbc-ljn":            ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "ljn-sbc":            ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "ypr-ljn":            ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "ljn-ypr":            ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "smvb-ljn":           ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "ljn-smvb":           ["CNB", "VGLJ", "BPL", "ET", "NGP", "SC", "BPQ", "GTL", "WADI", "JBP", "ORAI", "BZA"],
  "bengaluru-ernakulam": ["CBE", "ERS"],
  "bengaluru-coimbatore": ["SA", "CBE"],
  "bengaluru-madurai": ["SA", "MDU"],
  "bengaluru-ahmedabad": ["PUNE", "SUR", "BRC", "ST", "RTM"],
  "bangalore-ahmedabad": ["PUNE", "SUR", "BRC", "ST", "RTM"],
  ...
  // —— Bengaluru ↔ Guwahati deterministic corridor ——
  "bengaluru-guwahati": ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "guwahati-bengaluru": ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "bangalore-guwahati": ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "guwahati-bangalore": ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "bengaluru-ghy":      ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "ghy-bengaluru":      ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "bangalore-ghy":      ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "ghy-bangalore":      ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "bengaluru-kyq":      ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "kyq-bengaluru":      ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "bangalore-kyq":      ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "kyq-bangalore":      ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "sbc-ghy":           ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "ghy-sbc":           ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "ypr-ghy":           ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "ghy-ypr":           ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "smvb-ghy":          ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "ghy-smvb":          ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "sbc-kyq":           ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "kyq-sbc":           ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "ypr-kyq":           ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "kyq-ypr":           ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "smvb-kyq":          ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "kyq-smvb":          ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "sbc-guwahati":      ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "guwahati-sbc":      ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "ypr-guwahati":      ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  "guwahati-ypr":      ["NJP", "HWH", "BZA", "VSKP", "KGP", "MLDT", "BBS", "CTC", "NBQ", "PNBE", "NGP", "DDU"],
  ...
  // —— Bengaluru ↔ Jammu / Katra deterministic corridor ——
  "bengaluru-jammu":     ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "jammu-bengaluru":     ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "bengaluru-katra":     ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "katra-bengaluru":     ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "bangalore-jammu":     ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "jammu-bangalore":     ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "bangalore-katra":     ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "katra-bangalore":     ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "bengaluru-jat":       ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "jat-bengaluru":       ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "bengaluru-svdk":      ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "svdk-bengaluru":      ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "bangalore-jat":       ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "jat-bangalore":       ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "bangalore-svdk":      ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "svdk-bangalore":      ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "sbc-jat":             ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "jat-sbc":             ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "ypr-jat":             ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "jat-ypr":             ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "smvb-jat":            ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "jat-smvb":            ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "sbc-svdk":            ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "svdk-sbc":            ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "ypr-svdk":            ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "svdk-ypr":            ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "smvb-svdk":           ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "svdk-smvb":           ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "sbc-jammu":           ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "jammu-sbc":           ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "ypr-jammu":           ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "jammu-ypr":           ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "smvb-jammu":          ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "jammu-smvb":          ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "sbc-katra":           ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "katra-sbc":           ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "ypr-katra":           ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "katra-ypr":           ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "smvb-katra":          ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  "katra-smvb":          ["UMB", "NDLS", "LDH", "PTKC", "VGLJ", "BPL", "ET", "NGP", "SC", "GTL", "WADI", "BPQ"],
  ...
  "visakhapatnam-delhi": ["BZA", "NGP", "ET", "BPL"],
  ...
  "mumbai-indore": ["UJN", "RTM", "BRC", "ST", "NAD", "BPL", "ET", "BSL", "DWX", "GDA"],
  "indore-mumbai": ["UJN", "RTM", "BRC", "ST", "NAD"],
  "mumbai-indb":   ["UJN", "RTM", "BRC", "ST", "NAD", "BPL", "ET", "BSL", "DWX", "GDA"],
  "indb-mumbai":   ["UJN", "RTM", "BRC", "ST", "NAD"],
  "pune-delhi": ["MMR", "BSL", "ET", "BPL", "GWL", "AGC"],
...
class RouteMemoryStore {
  private memory = new Map<string, RouteMemoryEntry>();
  private readonly MAX_SIZE = 500;
  private readonly EXPIRY_MS = 30 * 24 * 60 * 60 * 1000; // 30 days

  public learn(source: string, destination: string, hub: string, waitMins: number, durationMins: number) {
    this.cleanup();
    ...
    if (existing) {
      ...
      if (this.memory.size >= this.MAX_SIZE) {
        ...
        if (oldestKey) {
          ...
          winstonLogger.debug(`[ROUTE_MEMORY_EXPIRE] LRU removed ${oldestKey}`);
        }
      }
      this.memory.set(key, {
        source,
    ...
    winstonLogger.debug(`[ROUTE_MEMORY_LEARN] source=${source} dest=${destination} hub=${hub} successCount=${this.memory.get(key)?.successCount}`);
  }

  public getBonus(source: string, destination: string, hub: string): number {
    this.cleanup();
    const key = `${source.toLowerCase()}-${destination.toLowerCase()}-${hub.toUpperCase()}`;
    const entry = this.memory.get(key);
    if (entry) {
      winstonLogger.debug(`[ROUTE_MEMORY_HIT] source=${source} dest=${destination} hub=${hub}`);
      return 120;
    }
    return 0;
  }
  ...
  private cleanup() {
    ...
    for (const [k, v] of this.memory.entries()) {
      if (now - v.lastUsedTimestamp > this.EXPIRY_MS) {
        ...
        winstonLogger.debug(`[ROUTE_MEMORY_EXPIRE] TTL removed ${k}`);
      }
    }
  }
}
...
// ——— Split Analytics & Monitoring Layer —————————————————————————————————————
export interface SplitAnalyticsMetrics {
  source: string;
  destination: string;
  hub: string | null;
  waitTime: number;
  ...
  cacheHit: boolean;
  memoryHit: boolean;
  status: 'SUCCESS' | 'EMPTY' | 'FAILED' | 'FALLBACK';
}

export class SplitAnalyticsMonitor {
  private events: any[] = [];
  private metrics: SplitAnalyticsMetrics[] = [];
  ...
  public trackSearch(metrics: SplitAnalyticsMetrics) {
  ...
  public getDashboardData() {
    ...
    const failedCorridors: Record<string, number> = {};
    let totalDur = 0;
    let totalWait = 0;
    let successCount = 0;
    let cacheHits = 0;
...
export const split_analytics = new SplitAnalyticsMonitor();
...
export class SplitJourneyEngine {
   ...
   */
  private static readonly MAJOR_JUNCTION_BYPASS = new Set<string>([
  ...
  private engineStartMs = 0;
  
  // Cache exclusively for split engine searches (3 minute TTL)
  private legSearchCache = new Map<string, { data: any[], timestamp: number }>();
  private legSearchStats = { hits: 0, misses: 0 };
   ...
   */
  private getMinHubDistance(sCode: string, dCode: string): number {
    const srcStation = ROUTE_STATIONS[sCode];
    const dstStation = ROUTE_STATIONS[dCode];
    if (srcStation && dstStation) {
      const directKm = this._calculateHaversine(
        srcStation.lat, srcStation.lng,
        dstStation.lat, dstStation.lng
      );
      if (directKm < 500)  return 50;   // Short: allow nearby junctions (e.g. Pune for Mumbai-Solapur)
      if (directKm < 1000) return 200;  // Medium: moderate filter
      return 300;                        // Long: near-original 350km behavior, prevents hub explosion
    }
    return MIN_HUB_DISTANCE_KM; // fallback constant (100km)
  }

  private async isWithinRange(src: string, via: string, dest: string, maxDetour: number = 300): Promise<boolean> {
    ...
    if (!cSrc || !cVia || !cDest) return true; // fallback allow if no coords
   ...
   */
  private async isNearAnyDestStation(
    via: string,
    dCodes: string[],
    thresholdKm: number
  ): Promise<boolean> {
    for (const dCode of dCodes) {
      const cVia = await stationService.getCoordinates(via);
      const cDest = await stationService.getCoordinates(dCode);
      if (!cVia || !cDest) continue;
  ...
  // PUBLIC ENTRY POINT
  // —————————————————————————————————————————————————————————————————————————
  private _splitRouteKey(source: string, destination: string, date: string): string {
    return `${source.toUpperCase().trim()}_${destination.toUpperCase().trim()}_${date}`;
  }

  async findCombinedRoutes(
    source: string,
    destination: string,
    date: string,
    directTrains?: Leg[],
    userId?: string,
    options?: SplitRouteOptions
    ...
    if (pending) {
      winstonLogger.info(`[SPLIT_ENGINE] Coalescing in-flight split search for ${routeKey}`);
      return pending;
    }
  ...
  private async _runFindCombinedRoutes(
    source: string,
    destination: string,
    date: string,
    directTrains?: Leg[],
    ...
    const t0 = Date.now();
    split_analytics.trackEvent('search_started', { source, destination, date });
    winstonLogger.debug(`[SPLIT_TRACE] ▶ findCombinedRoutes called: source=${source} destination=${destination} date=${date}`);
    try {
      ...
      const isTrainActive = (t: any, relaxedMode: boolean = false) => {
          if (!t) return false;
          ...
          if (num.length === 5 && /^[567]/.test(num)) {
            winstonLogger.debug(`[TRAIN_REJECTED_PASSENGER_SERIES] ${num}`);
            return false;
          }
          ...
          if (Array.isArray(t.classes) && t.classes.length > 0) {
            const hasClass = t.classes.some((c: any) => 
              (typeof c === 'string' ? c : c.class || '').toUpperCase() === requestedClass
            );
            if (!hasClass) {
              winstonLogger.debug(`[TRAIN_REJECTED_CLASS_MISMATCH] ${num} does not have requested class ${requestedClass}`);
              return false;
            }
          }
          ...
          if (SLEEPING_CLASSES.includes(requestedClass)) {
            ...
            if (CHAIR_CAR_KEYWORDS.some(kw => name.includes(kw))) {
              winstonLogger.debug(`[TRAIN_REJECTED_CLASS_MISMATCH] ${num} (${name}) is chair-car only; requested ${requestedClass}`);
              return false;
            }
          }
          ...
          if (requestedClass === 'SL') {
            ...
            if (PREMIUM_NO_SL_KEYWORDS.some(kw => name.includes(kw))) {
              winstonLogger.debug(`[TRAIN_REJECTED_CLASS_MISMATCH] ${num} (${name}) does not have SL class`);
              return false;
            }
          }
      ...
      const sanitizedSplits = rawSplits.filter(s => {
        ...
        if (!isTrainActive(l1) || !isTrainActive(l2)) {
          winstonLogger.debug(`[SPLIT_FILTER_CANCELLED] Rejected split due to inactive train: ${l1?.trainNo} or ${l2?.trainNo}`);
          rejectedCount++;
          return false;
        }
      ...
      if (sanitizedSplits.length === 0 && rawSplits.length > 0) {
        winstonLogger.warn(`[RELAXED_VALIDATION_RETRY] Sanitizer rejected all ${rawSplits.length} splits — retrying with relaxed mode`);
        const relaxedSeenCombos = new Set<string>();
        finalSanitized = rawSplits.filter(s => {
          if (!s || !s.hub || !s.legs || s.legs.length !== 2) return false;
          ...
          if (!isTrainActive(l1, true) || !isTrainActive(l2, true)) {
            winstonLogger.debug(`[RELAXED_VALIDATION_RETRY] Still rejected: ${l1?.trainNo} or ${l2?.trainNo} (cancelled/historical)`);
            return false;
          }

          if (typeof s.score !== 'number' || typeof s.totalDuration !== 'number') return false;
          const waitTime = s.wait_time ?? s.bufferMinutes;
          if (typeof waitTime !== 'number') return false;
          if (!l1 || !l2 || !l1.trainNo || !l2.trainNo || l1.trainNo === '00000' || l2.trainNo === '00000') return false;

          const comboKey = `${s.hub}_${l1.trainNo}_${l2.trainNo}`;
          if (relaxedSeenCombos.has(comboKey)) return false;
          relaxedSeenCombos.add(comboKey);

          return true;
        });
        winstonLogger.info(`[RELAXED_VALIDATION_RETRY] Recovered ${finalSanitized.length} splits in relaxed mode`);
      }
      ...
      const sameTrainBestMap = new Map<string, any>();
      for (const s of sameTrainSplits) {
        ...
        const sWait = s.wait_time != null ? s.wait_time : (s.bufferMinutes != null ? s.bufferMinutes : 9999);
        const eWait = existing ? (existing.wait_time != null ? existing.wait_time : (existing.bufferMinutes != null ? existing.bufferMinutes : 9999)) : Infinity;
        if (!existing || sWait < eWait) sameTrainBestMap.set(comboKey, s);
      }
      const dedupedSameTrain = [...sameTrainBestMap.values()];
      winstonLogger.info(`[DEDUP_SAME_TRAIN] sameTrain: ${sameTrainSplits.length}→${dedupedSameTrain.length}`);
      ...
      const sortedRegular = [...filteredRegular].sort((a: any, b: any) => {
      ...
      const isPremiumUser = !!(options as any)?.isPremiumUser;
      ...
      winstonLogger.debug(`[SPLIT_TRACE] ✅ findCombinedRoutes done in ${Date.now() - t0}ms | direct=${result.direct?.length} split=${result.split?.length}`);

      // —— STEP 8 — BACKEND QA LOGS ——
      winstonLogger.info(`[SPLIT_ENGINE] Accepted splits: ${finalSanitized.length}`);
      winstonLogger.info(`[SPLIT_ENGINE] Rejected splits: ${rejectedCount}`);
      if (finalSanitized.length !== sanitizedSplits.length) {
        winstonLogger.info(`[SAFE_VALIDATION_MODE] Original: ${sanitizedSplits.length} -> Relaxed recovery: ${finalSanitized.length}`);
      }

      if (finalSanitized.length === 0) {
        split_analytics.trackEvent('split_empty', { source, destination });
      } else {
        split_analytics.trackEvent('split_generated', { count: finalSanitized.length });
      }

      const responseTime = Date.now() - t0;
      const bestSplit = finalSanitized[0];
      const isCacheHit = !!(result as any)._isCacheHit; 
      const isMemoryHit = bestSplit && (bestSplit as any)._rankScore > 100;

      if (bestSplit) {
        split_analytics.trackSearch({
          source,
          destination,
      ...
      winstonLogger.error(`[SPLIT_TRACE] ❌ Engine threw: ${err?.message}`);
      throw err;
    }
  }

  // ——— INTERNAL IMPLEMENTATION —————————————————————————————————————————————
  private async _findCombinedRoutesInternal(
    source: string,
    destination: string,
    date: string,
    directTrainsRef?: Leg[],
    ...
    winstonLogger.info(`[SPLIT_ENGINE] Source stations: [${sCodes.join(',')}] Dest stations: [${dCodes.join(',')}]`);

    const fromCity = getCity(sCode);
    const toCity = getCity(dCode);

    winstonLogger.info(`[SPLIT_ENGINE] ${sName} [${sCodes.join(',')}] → ${dName} [${dCodes.join(',')}] on ${date}`);

    if (fromCity === toCity) {
      winstonLogger.info(`[SPLIT_ENGINE] Rejected: Source and destination are in the same city (${fromCity})`);
    ...
    for (const leg of directTrains) {
      const tNo = String((leg as any).trainNo || (leg as any).number || '').trim();
      const legFrom = String((leg as any).fromCode || (leg as any).from || (leg as any).fromStationCode || sCode || '').trim();
      const legTo = String((leg as any).toCode || (leg as any).to || (leg as any).toStationCode || dCode || '').trim();
      if (tNo && legFrom && legTo && await this.isProvenReverseScheduleSegment(tNo, legFrom, legTo)) {
        winstonLogger.debug(`[TRAIN_REJECTED_REVERSE_SN] direct ${tNo} ${legFrom}->${legTo}`);
        continue;
      }
    ...
    let splitJourneys: SplitJourney[] = [];
    let message = '';

    const engineStart = Date.now();
    winstonLogger.info(`[SPLIT_ENGINE] Running hub search (direct=${directTrains.length}, recommended=${shouldRecommendSplit})`);

    const segmentClass = options?.classType || '3A';
    const segmentQuota = options?.quota || 'GN';
    const { segmentAvailabilityEngine } = await import('./segmentAvailabilityEngine');
    const segmentSplitsPromise = segmentAvailabilityEngine
      ...
      .catch((e: any) => {
        winstonLogger.warn(`[SPLIT_ENGINE] Failed to resolve same-train segment splits: ${e.message}`);
        return [] as SplitJourney[];
      });
    ...
    const isExplicitlyInvalid = (leg: any): boolean => {
      ...
      if (!num || num === '00000' || num.length < 4) return true;
      if (['51411', '51412', '00000', '17321', '16332', '16340', '19023', '19024'].includes(num)) return true;
      if (name.includes('CANCEL') || name.includes('SUSPENDED') || name.includes('HISTORICAL')) return true;
      if (/^(PASSENGER|UNKNOWN EXPRESS|UNKNOWN TRAIN)\s*\d*/i.test(name.trim())) return true;

      return false;
    };

    // Helper: Validate leg structure (requires routing data, valid timings)
    const hasValidStructure = (leg: any): boolean => {
      const num = String(leg.trainNo || leg.number || '').trim();
      if (!leg.from || !leg.to || !leg.departure || !leg.arrival) {
        winstonLogger.debug(`[SPLIT_REJECTED] Leg ${num} missing routing data`);
        return false;
      }
      if (typeof leg.durationMins === 'number' && isNaN(leg.durationMins)) {
        winstonLogger.debug(`[SPLIT_REJECTED] Leg ${num} has NaN duration`);
        return false;
      }
      return true;
    };
    ...
    const strictValidatedSplits: SplitJourney[] = [];
    for (const split of rawSplits) {
      if (!split || !split.legs || split.legs.length !== 2) continue;

      let validSplit = true;
      for (const leg of split.legs as any[]) {
        const num = String(leg.trainNo || leg.number || '').trim();

        // Reject explicitly invalid trains
        if (isExplicitlyInvalid(leg)) {
          winstonLogger.debug(`[STRICT_VALIDATION] Rejecting ${num} — explicitly invalid`);
          validSplit = false;
          break;
        }
        ...
        if (isFabricated) {
          const dbT = (dbTrains as any[]).find((t: any) => String(t.trainNo) === num || String(t.number) === num);
          if (dbT && dbT.name && !/^(Passenger|Unknown Express|Unknown Train|Train)\s*\d*/i.test(dbT.name)) {
             tName = dbT.name;
             leg.trainName = tName;
             leg.name = tName;
             winstonLogger.debug(`[STRICT_VALIDATION] Hydrated ${num}: ${tName}`);
          } else {
             // In strict mode, missing name fails validation
             winstonLogger.debug(`[STRICT_VALIDATION] Rejecting ${num} — name unavailable`);
             validSplit = false;
             break;
          }
        }
        ...
        if (typeof split.totalDuration === 'number' && isNaN(split.totalDuration)) {
          winstonLogger.debug(`[STRICT_VALIDATION] Rejecting split hub ${split.hub} — NaN duration`);
          validSplit = false;
          break;
        }
      }
    ...
    if (safeSplits.length === 0 && rawSplits.length > 0) {
      winstonLogger.info(`[RELAXED_VALIDATION_MODE] Strict validation found 0 splits from ${rawSplits.length} — activating relaxed fallback`);

      const relaxedValidatedSplits: SplitJourney[] = [];
      for (const split of rawSplits) {
        if (!split || !split.legs || split.legs.length !== 2) continue;
        ...
        for (const leg of split.legs as any[]) {
          ...
          if (isExplicitlyInvalid(leg)) {
            winstonLogger.debug(`[RELAXED_VALIDATION] Rejecting ${num} — explicitly invalid`);
            validSplit = false;
            break;
          }
          ...
          if (!hasValidStructure(leg)) {
            winstonLogger.debug(`[RELAXED_VALIDATION] Rejecting ${num} — invalid structure`);
            validSplit = false;
            break;
          }
          ...
          if (isFabricated) {
            ...
            if (dbT && dbT.name && !/^(Passenger|Unknown Express|Unknown Train|Train)\s*\d*/i.test(dbT.name)) {
               ...
               leg.name = dbT.name;
               winstonLogger.debug(`[RELAXED_VALIDATION] Hydrated ${num}: ${dbT.name}`);
            } else {
               ...
               winstonLogger.debug(`[RELAXED_VALIDATION] Allowing ${num} without hydration`);
            }
          }
        }
      ...
      if (relaxedValidatedSplits.length > 0) {
        winstonLogger.info(`[RELAXED_VALIDATION_MODE] ✅ Recovered ${relaxedValidatedSplits.length} splits using relaxed validation`);
        safeSplits = relaxedValidatedSplits;
      }
    } else if (safeSplits.length > 0 && rawSplits.length > safeSplits.length) {
      winstonLogger.info(`[RELAXED_RECOVERY_MODE] Strict validation verified ${safeSplits.length} splits. Running relaxed validation on remaining ${rawSplits.length - safeSplits.length} candidates.`);

      const remainingCandidates = rawSplits.filter(s => !strictValidatedSplits.includes(s));
      const recoveredSplits: SplitJourney[] = [];
      for (const split of remainingCandidates) {
        ...
        for (const leg of split.legs as any[]) {
          ...
          // RELAXED: Still reject explicitly invalid trains
          if (isExplicitlyInvalid(leg)) {
            winstonLogger.debug(`[RELAXED_RECOVERY] Rejecting ${num} — explicitly invalid`);
            validSplit = false;
            break;
          }
          ...
          // RELAXED: Still require valid structure (routing + timings)
          if (!hasValidStructure(leg)) {
            winstonLogger.debug(`[RELAXED_RECOVERY] Rejecting ${num} — invalid structure`);
            validSplit = false;
            break;
          }
          ...
          if (isFabricated) {
            ...
            if (dbT && dbT.name && !/^(Passenger|Unknown Express|Unknown Train|Train)\s*\d*/i.test(dbT.name)) {
               leg.trainName = dbT.name;
               leg.name = dbT.name;
               winstonLogger.debug(`[RELAXED_RECOVERY] Hydrated ${num}: ${dbT.name}`);
            } else {
               if (!tName || tName.length === 0) {
                 leg.trainName = `Train ${num}`;
                 leg.name = `Train ${num}`;
               }
               winstonLogger.debug(`[RELAXED_RECOVERY] Allowing ${num} without hydration`);
            }
          }
        }
      ...
      if (recoveredSplits.length > 0) {
        winstonLogger.info(`[RELAXED_RECOVERY_MODE] ✅ Recovered ${recoveredSplits.length} additional splits`);
    ...
    winstonLogger.info(`[SPLIT_ENGINE] Same-train segment splits found: ${sameTrainSplits.length}`);
      ...
      try {
        ...
        if (existingCache) {
          await supabase
            .from('api_search_cache')
            .update(cachePayload)
            .eq('id', (existingCache as any).id);
        } else {
          await supabase
            .from('api_search_cache')
            .insert(cachePayload);
        }
        winstonLogger.info(`[SPLIT_ENGINE] 🗄️ Saved splits to database cache for ${source}→${destination} under ${dbKey}`);
      } catch (dbErr: any) {
        winstonLogger.warn(`[SPLIT_DB_CACHE] Write failed: ${dbErr.message}`);
      }
    } else {
      winstonLogger.info(`[SPLIT_ENGINE] ⏭️ NOT caching empty result to allow retry on next query`);
    }
    ...
    if (safeSplits.length === 0 && elapsed > 12000) {
      winstonLogger.info(`[SPLIT_ENGINE] Timeout approaching (elapsed: ${elapsed}ms) with 0 splits. Attempting adjacent-date cache fallback.`);
      ...
      for (const adjDate of [prevDate, nextDate]) {
        ...
        try {
          ...
          winstonLogger.warn(`[SPLIT_DB_CACHE] Adjacent read failed for ${adjDate}: ${dbErr.message}`);
        }
      }
    }

    if (process.env.NODE_ENV !== 'production') console.log(`[SPLIT_ENGINE] Stats: ${this.legSearchStats.hits} cache hits, ${this.legSearchStats.misses} cache misses`);
    return result;
  }
   ...
   */
  private async resolveCityStations(cityOrCode: string): Promise<string[]> {
    if (!cityOrCode || typeof cityOrCode !== 'string') return [];
    try {
      return await stationService.getStationsForCity(cityOrCode);
    } catch (err: any) {
      winstonLogger.error(`[STATION_RESOLVE] DB lookup failed for "${cityOrCode}": ${err.message}`);
      return [stationService.normalizeInput(cityOrCode)];
    }
  }
  ...
  // DIRECT TRAINS
  // —————————————————————————————————————————————————————————————————————————
  private async getDirectTrainsForCity(sCodes: string[], dCodes: string[], date: string): Promise<Leg[]> {
    if (this.apiCallCount >= this.MAX_TOTAL_CALLS) return [];

    const pairs: Array<{ s: string; d: string }> = [];
    ...
    const allRawTrains: Array<{ t: any; s: string; d: string }> = [];
    ...
    const uniqueRawTrains: Array<{ t: any; s: string; d: string }> = [];
    ...
    const enriched = await Promise.all(
      topTrains.map(async (item) => {
        const { t, s, d } = item;
        if (this.apiCallCount >= this.MAX_TOTAL_CALLS) return this.mapToLeg(t, s, d);

        this.apiCallCount++;
        const trainNo = t.train_number || t.trainNo || t.number;

        const availRes = await availabilityProvider.getAvailability({
          trainNo,
          from: s,
          to: d,
          date,
    ...
    return enriched.filter(Boolean) as Leg[];
  }
   ...
   */
  private checkSplitRecommendation(direct: Leg[]): boolean {
    ...
    if (direct.length < 3) return true;
    ...
    for (const train of direct) {
      const availText = (train as any).availability?.status ||
                        (train as any).availability?.current_status ||
                        (train as any).availabilityStatus || '';
      const isWaitlisted = WL_PATTERNS.test(String(availText).trim());
      if (!isWaitlisted) confirmedCount++;
    }

    // If 3 or more direct trains have confirmed seats, split is not urgent
    if (confirmedCount >= 3) {
      winstonLogger.info(`[SPLIT_RECOMMEND] ${confirmedCount} direct trains with confirmed seats — split not strictly needed`);
      return false;
    }

    winstonLogger.info(`[SPLIT_RECOMMEND] Only ${confirmedCount}/${direct.length} direct trains confirmed — recommending split`);
    return true;
  }
  ...
  private async handleGptOptimizedRoute(
    sName: string, dName: string, sCodes: string[], dCodes: string[], date: string
    ...
    try {
      ...
      if (!gptRoute?.route) return [];
      const hub = gptRoute.route.split('→')[1]?.trim();
      if (!hub) return [];
      winstonLogger.info(`[GPT_SPLIT] Suggested hub: ${hub}`);
      return await this.findSplitJourneys(sName, dName, sCodes, dCodes, date, undefined, [hub]);
    } catch (err: any) {
      winstonLogger.warn(`[GPT_SPLIT] Failed: ${err.message}`);
      return [];
    }
  }
   ...
   */
  private async getDynamicHubs(sourceCode: string, destCode: string): Promise<string[]> {
    try {
      const { supabase } = await import('../config/supabase');
      ...
      if (!sTrains?.length || !dTrains?.length) return [];

      const sNos = [...new Set(sTrains.map((t: any) => String(t.Train_No)))];
      const dNos = [...new Set(dTrains.map((t: any) => String(t.Train_No)))];
      ...
      if (!l1Raw?.length) return [];

      const l1Stations = [...new Set(l1Raw.map((t: any) => String(t.Station_Code)))]
      ...
      const l2TrainNos = new Set<string>();
      const CHUNK = 50;
      for (let i = 0; i < Math.min(l1Stations.length, 200); i += CHUNK) {
        ...
        (hop2 || []).forEach((t: any) => l2TrainNos.add(String(t.Train_No)));
      }
      ...
      const l2Stations = new Set((l2Raw || []).map((t: any) => String(t.Station_Code)));
      ...
      const dSet = new Set((dStations || []).map((t: any) => String(t.Station_Code)));
      ...
      winstonLogger.info(
        `[DYNAMIC_HUB] 2-hop graph: L1=${l1Stations.length} stations, ` +
        `L2 trains=${hop2Arr.length}, hubs found=${hubs.length}`
      );

      return hubs;
    } catch (e: any) {
      winstonLogger.warn(`[DYNAMIC_HUB] Failed: ${e.message}`);
      return [];
    }
  }
  ...
  // —————————————————————————————————————————————————————————————————————————
  private async findSplitJourneys(
    sName: string, dName: string,
    sCodes: string[], dCodes: string[],
    date: string, directTrainsRef?: Leg[], providedHubs?: string[]
  ): Promise<SplitJourney[]> {
    ...
    if (isDeterministic) {
      ...
      winstonLogger.info(`[SPLIT_ENGINE] Deterministic-priority corridor ${pairKey1}: priority=[${deterministicHubs.join(',')}] total=${hubs.length}`);
    } else {
    ...
    winstonLogger.info(`[HUB_SELECT] After blacklist: ${hubs.length} hubs (city=${sourceCity})`);
        ...
        if (!error && data) {
          data.forEach((row: any) => {
            ...
            if (row.latitude && row.longitude) {
              ...
              const cacheKey = `station_info_${code}`;
              cacheService.set(cacheKey, row, 3600);
            }
          });
        }
      } catch (err: any) {
        winstonLogger.warn(`[BATCH_PRELOAD] Failed: ${err.message}`);
      }
    }
    ...
    const getCoordsFallbackLocal = (code: string) => {
      ...
      if (re) return { lat: re.lat, lon: re.lng };
      return null;
    };

    const isNearAnyDestStationLocal = (
      via: string,
      destCodes: string[],
      thresholdKm: number
    ): boolean => {
    ...
    if (srcCoords) {
      ...
      for (const h of hubs) {
        ...
        if (!hCoords) { distFiltered.push(h); continue; } // allow if no coords
        ...
        if (distKm >= minDist) {
          ...
          distFiltered.push(h);
          winstonLogger.info(`[HUB_ACCEPT_JUNCTION] ${h} is ${Math.round(distKm)}km from source — below generic floor (${minDist}km) but accepted as designated major junction hub`);
        } else {
          winstonLogger.info(`[HUB_REJECT] ${h} is only ${Math.round(distKm)}km from source — below ${minDist}km minimum floor`);
        }
      }
      hubs = distFiltered;
    }
    ...
    for (let i = 0; i < hubPool.length; i += BATCH_SIZE) {
      ...
      const batchResults = await Promise.all(
        batch.map(async (hub) => {
          try {
            const hubCodes = await stationService.getStationsForCity(hub);
    ...
    winstonLogger.info(`[SPLIT_ENGINE] Phase 1 done: ${viableHubs.length}/${hubPool.length} hubs have leg1 trains`);
    ...
    const directTime = directTrainsForFilter.length > 0 ?
      Math.min(...directTrainsForFilter.map(t => {
        ...
        const numericMins = typeof (t as any).durationMins === 'number' && (t as any).durationMins > 0
          ? (t as any).durationMins
          : typeof (t as any).duration_mins === 'number' && (t as any).duration_mins > 0
            ? (t as any).duration_mins
            : typeof (t as any).duration === 'number' && (t as any).duration > 0
              ? (t as any).duration
              : 0;
        if (numericMins > 0) return numericMins;

        // 2) Parse string duration — handles "26:15", "26:15 hrs", "26h 15m" etc.
        const rawDurStr: string = String(
          (t as any).total_journey_time ||
          (t as any).duration_str ||
          (t as any).durationStr ||
          (t as any).journeyDuration ||
          (t as any).duration ||
          ''
        );
        ...
        const dep = (t as any).departure || (t as any).departure_time || '';
        const arr = (t as any).arrival || (t as any).arrival_time || '';
        const dayNum = parseInt((t as any).day_number || (t as any).dayNumber || '1') || 1;
        if (dep && arr) {
    ...
    winstonLogger.info(`[SPLIT_ENGINE] directTime=${directTime === Infinity ? 'Infinity (no direct trains)' : directTime + 'min'}`);
    ...
    if (isReverseLoopGlobal) {
      winstonLogger.info(`[SPLIT_ENGINE] Search rejected: Destination is within 50km of source`);
      return [];
    }
    ...
    for (const { hCode, hName, trains: leg1Raw } of viableHubs) {
      ...
      if (hubIsDestination) continue; // already handled in Phase 2 skip above

      let combosThisHub = 0;

      // Collect all leg2 results across all destination station codes
      const validLeg2Pools: Array<{ leg2Raw: any[]; leg2Date: string; dC: string }> = [];
      let totalLeg2 = 0;
      for (const dC of dCodes) {
        const isLeg2SameCity = isNearAnyDestStationLocal(dC, [hCode], 20);
        if (isLeg2SameCity) continue;
      ...
      for (const t1 of leg1Raw) {
        const actualSCode = (t1 as any)._fromCode || sCode;
        ...
        for (const { leg2Raw, leg2Date, dC } of validLeg2Pools) {
          if (combosThisHub >= this.MAX_COMBOS_PER_HUB) break;
          ...
          for (const t2 of leg2Raw) {
            ...
            if (!l1.arrival || l1.arrival === '--:--' || !l2.departure || l2.departure === '--:--') continue;
            ...
            if (waitMins >= 120 && waitMins <= 720) {       // 2h–12h = LOW (includes overnight)
              success_percent = 90; risk_level = 'LOW';
            } else if (waitMins >= 45 && waitMins < 120) {  // 45m–2h = MEDIUM (tight connection)
              success_percent = 65; risk_level = 'MEDIUM';
            } else {                                         // >12h or short = MEDIUM (very long layover)
              success_percent = 60; risk_level = 'MEDIUM';
            }
            ...
            const combo: SplitJourney = {
              ...
              risk_level,
              ai_reason,
              total_duration: rankingService.formatDuration(totalMins),
              leg1_duration: rankingService.formatDuration(leg1Duration),
              leg2_duration: rankingService.formatDuration(leg2Duration),
              wait_formatted: rankingService.formatDuration(waitMins),
              wait_time: waitMins,
              steps: [
            ...
            analyticsService.logHubSuccess(hName);
            winstonLogger.info(
              `[SPLIT_ENGINE] ✅ ${hCode}: ${l1.trainNo}+${l2.trainNo} wait=${waitMins}m total=${rankingService.formatDuration(totalMins)}`
            );
          }
        }
      }
      ...
      if (combosThisHub > 0) {
        winstonLogger.info(`[SPLIT_ENGINE] Hub ${hCode}: ${combosThisHub} combos`);
      }
    }

    const preFilterCount = allCombinations.length;
    winstonLogger.debug(`[SPLIT_TRACE] Generated combinations (preFilter): ${preFilterCount}`);
    winstonLogger.info(`[SPLIT_ENGINE] TOTAL COMBINATIONS GENERATED: ${preFilterCount}`);
    if (preFilterCount === 0) {
      winstonLogger.warn('[SPLIT_ENGINE] ⚠️  ZERO combos generated. Check hub connectivity and train schedules.');
      // removed debug log
    }
    ...
    winstonLogger.debug(`[SPLIT_TRACE] preFilterCount=${preFilterCount} postFilterCount=${postFilterCount} relaxedCount=${relaxedCombinations.length}`);
    ...
    const hubModifiers = new Map<string, number>();
    for (const combo of filteredCombinations) {
      const h = (combo as any).hub || '';
    ...
    const withScore = filteredCombinations.map(c => {
      ...
      let trainBonus = 0;
      const getTrainPriority = (name: string) => {
        const lower = (name || '').toLowerCase();
        if (lower.includes('rajdhani') || lower.includes('duronto') || lower.includes('vande bharat')) return 50;
        if (lower.includes('sf') || lower.includes('superfast') || lower.includes('shatabdi')) return 30;
        if (lower.includes('express') || lower.includes('mail')) return 10;
        if (lower.includes('passenger') || lower.includes('memu') || lower.includes('demu') || lower.includes('local')) return -100;
        return 0;
      };
      ...
      let hubBonus = 0;
      let tier = 'STANDARD';
      const hubCode = (c.leg1 as any)?.toCode || (c.leg2 as any)?.fromCode || '';
      ...
      try {
        ...
        winstonLogger.error(`[DETOUR_PENALTY_ERROR] ${e.message}`);
      }

      return {
        ...c,
        _rankScore:
          (c.success_percent ?? 0) * 1.5
          - waitPenalty
    ...
    winstonLogger.info(`[SPLIT_ENGINE] directTime=${Math.round(directTimeH)}h → maxSplits=${MAX_SPLIT_RESULTS}`);
    ...
    winstonLogger.info(`[SPLIT_ENGINE] Enforcing safe live running-day validation...`);
    const liveValidatedCombos: typeof withScore = [];
    const searchPromises = new Map<string, Promise<any[] | null>>();
    const getLiveTrains = (f: string, t: string, dt: string) => {
      const key = `${f}_${t}_${dt}`;
      if (!searchPromises.has(key)) {
         searchPromises.set(key, irctcService.search(f, t, dt).catch(() => null));
      }
      return searchPromises.get(key) as Promise<any[] | null>;
    };
    ...
    const validationResults = await Promise.all(topCandidates.map(async (c) => {
       try {
           const leg1From = (c as any).leg1.fromCode || (c as any).leg1.fromStationCode || (c as any).leg1.from || sCode;
           const leg1To = (c as any).leg1.toCode || (c as any).leg1.toStationCode || (c as any).leg1.to || (c as any).hub;
           const leg1Date = (c as any).leg1.travelDate || date;
           ...
           if (l1Live !== null && Array.isArray(l1Live) && l1Live.length >= 2) {
               ...
               winstonLogger.debug(`[TRAIN_ALLOWED_METADATA_MISSING] Leg1 ${(c as any).leg1.trainNo} — live API unavailable, allowing`);
           }
           ...
           if (l2Live !== null && Array.isArray(l2Live) && l2Live.length >= 2) {
               ...
               // API unavailable or empty — ALLOW by default
               winstonLogger.debug(`[TRAIN_ALLOWED_METADATA_MISSING] Leg2 ${(c as any).leg2.trainNo} — live API unavailable, allowing`);
           }
           return c;
       } catch (err) {
           // On error, ALLOW the candidate (safe default)
           winstonLogger.debug(`[SAFE_VALIDATION_MODE] Live validation error — allowing candidate`);
           return c;
       }
    }));

    for (const v of validationResults) {
        if (v) liveValidatedCombos.push(v);
    }

    // Max 2 results per hub to ensure hub diversity across the result set
    const hubCount = new Map<string, number>();
    const finalSplits: SplitJourney[] = [];
    for (const c of liveValidatedCombos) {
      const h = (c as any).hub || '';
      const cnt = hubCount.get(h) || 0;
      if (cnt >= 2) continue;       // max 2 from a single hub
    ...
    if (finalSplits.length > 0) {
      ...
      finalSplits.forEach((route, index) => {
        ...
        if (index === 0) uiBadges.push('⭐ Recommended Hub: ' + (route.hub || ''));
        if (avgSeatProbability >= 85) uiBadges.push('✅ Strong Confirmation Corridor');
        if ((route.bufferMinutes || 0) <= 60 && (route.bufferMinutes || 0) >= 15) uiBadges.push('⚡ Fastest Transfer');
        if (waitH >= 1 && waitH <= 3) uiBadges.push('🎫 Best for Tatkal');
        if ((route.bufferMinutes || 0) >= 120) uiBadges.push('🛡️ Recommended Buffer');
    ...
    winstonLogger.debug(`[SPLIT_TRACE] VALID SPLITS AFTER RANKING: ${finalSplits.length} (raw: ${preFilterCount} postFilter: ${postFilterCount})`);
    winstonLogger.info(`[SPLIT_ENGINE] VALID SPLITS AFTER RANKING: ${finalSplits.length} (raw: ${preFilterCount})`);
    ...
    const validatedSplits = finalSplits.filter(split => {
      ...
      const fromCode = (split.legs[0] as RichLeg).fromCode || '';
      const toCode = (split.legs[1] as RichLeg).toCode || '';
    ...
    const debugData: SplitDebugData = {
    ...
    if (filteredSplits.length === 0) {
      ...
      if (validatedFallback.length > 0) {
        ...
        // If they didn't pass strict day/flow validation, they should not be shown.
          ...
          const fallbackSplits: SplitJourney[] = [];

          if (isDeterministic) {
            winstonLogger.debug(`[SPLIT_TRACE] Deterministic corridor enforced. Bypassing forced major hub fallback.`);
          } else {
            ...
            winstonLogger.debug(`[SPLIT_TRACE] FORCED_HUB_FALLBACK: trying ${forcedMajorHubs.length} major hubs: [${forcedMajorHubs.join(', ')}]`);

            for (const hub of forcedMajorHubs) {
              if (sCodes.includes(hub) || dCodes.includes(hub)) continue;

              try {
                winstonLogger.debug(`[SPLIT_TRACE] FALLBACK_HUB: trying ${hub}`);
                ...
                winstonLogger.debug(`[SPLIT_TRACE] FALLBACK_HUB ${hub}: found ${hubSplits.length} splits`);
                if (hubSplits.length > 0) {
                  fallbackSplits.push(...hubSplits.slice(0, 2));
                  debugData.fallbackStrategy = 'forced-major-hub';
                  winstonLogger.info(`[SPLIT_ENGINE] Found ${hubSplits.length} splits through forced hub ${hub}`);
                  if (fallbackSplits.length >= 4) break;
                }
              } catch (error: any) {
                winstonLogger.warn(`[SPLIT_ENGINE] Failed to find splits through forced hub ${hub}: ${error.message}`);
      ...
      if (filteredSplits.length === 0) {
        winstonLogger.warn('[SPLIT_ENGINE] ⚠️  No splits found even after all fallbacks — returning empty array');
        // removed debug log
        debugData.finalSplitCount = 0;
        SplitDebugLogger.log(debugData);
        return [];
      }
    // Log debug data
    ...
    const validateLegSync = (leg: any, rejectedReasonRef: { value: string }, legIndex: number): boolean => {
      if (!leg) { rejectedReasonRef.value = `Leg ${legIndex} missing`; return false; }
      if (!leg.trainNo || leg.trainNo === "00000") { rejectedReasonRef.value = `Leg ${legIndex} missing trainNo`; return false; }
      if (!leg.departure || leg.departure === "--:--") { rejectedReasonRef.value = `Leg ${legIndex} missing departure`; return false; }
      if (!leg.arrival || leg.arrival === "--:--") { rejectedReasonRef.value = `Leg ${legIndex} missing arrival`; return false; }
      let realTrainReason = '';
      const realTrainResult = this.isRealTrain(leg, (reason) => { realTrainReason = reason; });
      if (!realTrainResult) { rejectedReasonRef.value = `Leg ${legIndex} failed isRealTrain: ${realTrainReason}`; return false; }
      if (!this.isTrainActive(leg)) { rejectedReasonRef.value = `Leg ${legIndex} failed isTrainActive`; return false; }
      return true;
    };
    ...
    for (const split of splitsToNormalize) {
      ...
      if (!validateLegSync(leg1, ref, 1) || !validateLegSync(leg2, ref, 2)) {
        if (process.env.NODE_ENV !== 'production') console.log(`[REAL_AUDIT] VALIDATE_REJECT_SYNC | Hub: ${split.hub} | Reason: ${ref.value}`);
        continue;
      }
      ...
      if (process.env.NODE_ENV !== 'production') console.log(`[REAL_AUDIT] VALIDATED_ACCEPTED | Hub: ${split.hub} | Leg1: ${leg1.trainNo} (${leg1.fromCode}->${leg1.toCode}) | Leg2: ${leg2.trainNo} (${leg2.fromCode}->${leg2.toCode})`);
    ...
    winstonLogger.info(
      `[SPLIT_ENGINE] ✅ ${normalizedSplits.length} valid splits | raw=${allCombinations.length} | ` +
    ...
    if (process.env.NODE_ENV !== 'production') console.log(`[SPLIT_FINAL] Total candidates: ${allCombinations.length}, Accepted: ${normalizedSplits.length}`);
  ...
  private isTrainActive(leg: any): boolean {
    if (!leg) return false;
    ...
    if (!num || num === '00000' || num.length < 4) {
      winstonLogger.debug(`[TRAIN_REJECTED_NOT_FOUND] ${num} - invalid train number`);
      return false;
    }
    ...
    if (isCancelled || isSuspended || isHistorical || noRunningDays) {
      winstonLogger.debug(`[SPLIT_FILTER_CANCELLED] Rejecting inactive train: ${leg.trainNo || leg.train_number} - Status: ${status}`);
      return false;
    }
    return true;
  }
  ...
  private isRealTrain(train: any, logCb?: (reason: string) => void): boolean {
    if (!train) return false;

    // Number — raw OR normalized
    const num = String(
      train.train_number || train.trainNo || train.train_no || train.trainNumber || train.number || ''
    ).trim();
    if (!num || num === '00000' || num.length < 4) {
      if (logCb) logCb("invalid trainNo");
      return false;
    }

    // Name — raw OR normalized; reject fabricated placeholders AND missing/N/A names
    const name = String(
      train.train_name || train.trainName || train.name || ''
    ).trim();
    if (!name || name === 'N/A' || /^(Passenger|Unknown Express|Unknown Train|Train)\s*\d*/i.test(name)) {
      winstonLogger.warn(`[TRAIN_NAME_WARNING] ${num} - Fabricated/missing name: "${name}" (Bypassing for live route)`);
    ...
    if (!dep || !arr || dep === '--:--' || arr === '--:--') {
      winstonLogger.warn(`[TRAIN_REJECTED_NO_TIMETABLE] ${num} - Missing timings (dep=${dep}, arr=${arr})`);
      if (logCb) logCb("invalid timing");
      return false;
    }
    ...
    if (stops !== undefined && (!Array.isArray(stops) || stops.length < 2)) {
      winstonLogger.warn(`[TRAIN_REJECTED_NO_TIMETABLE] ${num} - Insufficient stops (${Array.isArray(stops) ? stops.length : 'non-array'})`);
      if (logCb) logCb("missing timetable");
      return false;
    }

    winstonLogger.info(`[TRAIN_VERIFIED_REAL] ${num} - "${name}" passed validation`);
    return true;
  }

  // ── FINAL LIVE SPLIT RECOVERY ──
  private async recoverValidLeg(from: string, to: string, date: string): Promise<any | null> {
    winstonLogger.info(`[LEG_RECOVERY_STARTED] Attempting live recovery for ${from} -> ${to} on ${date}`);
      ...
      if (Array.isArray(apiResult) && apiResult.length > 0) {
        ...
        for (let t of apiResult) {
          ...
          if (!isReal && recoveryFailReason === "missing name") {
             ...
             if (trainNo && trainNo !== '00000') {
                 try {
                     let officialName: string | undefined | null = null;
                     ...
                     if (!officialName || officialName === 'N/A') {
                        officialName = await dbService.dbLookupTrainName(trainNo);
                     }
                     ...
                     if (officialName && officialName !== 'N/A' && !/^(Passenger|Unknown Express|Unknown Train|Train)\s*\d*/i.test(officialName)) {
                         ...
                         t.name = officialName;
                         winstonLogger.info(`[TRAIN_NAME_HYDRATED] Hydrated ${trainNo} with official name: ${officialName}`);
        ...
        if (validTrains.length > 0) {
          ...
          winstonLogger.info(`[LEG_RECOVERY_SUCCESS] Recovered ${recovered.train_number || recovered.trainNo || recovered.train_no || recovered.trainNumber || recovered.number} for ${from}->${to}`);
          
          // Cache ONLY verified recovered trains
          dbService.saveSearchToDB({ source: from, destination: to, date, trains: [recovered], api_used: "recovery_live" }).catch(() => {});
          
          return recovered;
        } else {
          winstonLogger.warn(`[LEG_RECOVERY_FAILED] ${from}->${to} Error: ${recoveryFailReason}`);
        }
      } else if (apiResult === null) {
          winstonLogger.warn(`[LEG_RECOVERY_FAILED] ${from}->${to} Error: live lookup failed (timeout)`);
      } else {
          winstonLogger.warn(`[LEG_RECOVERY_FAILED] ${from}->${to} Error: live lookup failed (empty array)`);
      }
    } catch (e: any) {
      winstonLogger.warn(`[LEG_RECOVERY_FAILED] ${from}->${to} Error: live lookup failed (${e.message})`);
      return null;
    }
    
    winstonLogger.warn(`[LEG_RECOVERY_FAILED] No real live trains found for ${from}->${to}`);
    return null;
  }
  ...
  private async validateLegAndCorrectAsync(
    ...
    role: 'leg1' | 'leg2',
    date: string
  ): Promise<{ isValid: boolean; correctedLeg?: any; reason?: string }> {
    ...
    if (!toStop) {
      ...
      if (matchingTo) {
        ...
        winstonLogger.info(`[TERMINAL_CORRECTION] Corrected Leg toCode for train ${num}: ${currentTo} -> ${matchedCode}`);
      }
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────
  private normalizeTrain(leg: any): any {
    const safe = (v: any, d = 'N/A') =>
      v !== undefined && v !== null && v !== '' ? v : d;

    return {
      // —— identity ——
      trainNo: normalizeTrainNumber(String(safe(leg.trainNo || leg.train_number || leg.train_no || leg.trainNumber || leg.number || leg.train?.number || leg.train?.trainNo, '00000'))),
      trainName: (() => {
        let name = safe(leg.trainName || leg.train_name || leg.name || leg.train?.name || leg.train?.trainName, undefined);
        if (!name) {
          const tNo = normalizeTrainNumber(String(safe(leg.trainNo || leg.train_number || leg.train_no || leg.trainNumber || leg.number || leg.train?.number || leg.train?.trainNo, '00000')));
          try {
            const dbTrains = require('../data/dbTrains.json');
            const dbEntry = (dbTrains as any[]).find((t: any) => String(t.trainNo) === tNo || String(t.number) === tNo);
            if (dbEntry) name = dbEntry.name || dbEntry.trainName || dbEntry.train_name;
          } catch(e) {}
        }
        return name;
      })(),
   ...
   */
  private async isProvenReverseScheduleSegment(trainNo: string, from: string, to: string): Promise<boolean> {
    const num = normalizeTrainNumber(String(trainNo || '').trim());
    const fromIn = String(from || '').toUpperCase().trim();
    const toIn = String(to || '').toUpperCase().trim();
    if (!num || num === '00000' || !fromIn || !toIn) return false;

    const scheduleCacheKey = `train_schedule_resolved_${num}`;
    let stops: Array<{ Station_Code: string; SN: number }> = [];
    const cached = cacheService.get<{ stops: any[] }>(scheduleCacheKey);
  ...
  private async filterProvenReverseTrains(trains: any[], from: string, to: string): Promise<any[]> {
    const out: any[] = [];
    for (const t of trains) {
      ...
      if (tNo && await this.isProvenReverseScheduleSegment(tNo, from, to)) {
        winstonLogger.debug(`[TRAIN_REJECTED_REVERSE_SN] ${tNo} ${from}->${to}`);
        continue;
      }
      out.push(t);
    }
  ...
  private async searchLeg(from: string, to: string, date: string, forceDbFallback = false) {
    ...
    if (cached && Date.now() - cached.timestamp < 180000) { // 3 min cache
      winstonLogger.debug(`[SPLIT_SEARCH_CACHE_HIT] ${cacheKey}`);
    ...
    if (hasLiveBudget) {
      ...
      try {
        ...
        if (Array.isArray(apiResult) && apiResult.length > 0) {
          const verified = apiResult.filter(isBasicallyValid).map((t: any) => ({
            ...t,
            travelDate: date
          }));
          ...
          winstonLogger.info(`[SPLIT_SEARCH_TIMEOUT] Live API timed out for ${from}->${to} - falling back to DB`);
        }
      } catch (err: any) {
        winstonLogger.warn(`[TRAIN_REJECTED_LIVE_LOOKUP] ${from}->${to} live API error: ${err?.message} - falling back to DB`);
      }
    } else {
      if (forceDbFallback) {
        winstonLogger.debug(`[SPLIT_SEARCH_DB_ONLY] Forced DB fallback for secondary terminal ${from}->${to}`);
      } else {
        winstonLogger.info(`[SPLIT_SEARCH_SKIP] Live budget exhausted (${elapsed}ms) - using DB fallback for ${from}->${to}`);
      }
    }
    ...
    try {
      ...
      if (Array.isArray(dbResult) && dbResult.length > 0) {
        ...
        if (forwardOnly.length > 0) {
          winstonLogger.info(`[SPLIT_SEARCH_DB] DB fallback hit: ${from}->${to} = ${forwardOnly.length} trains (live unavailable)`);
          this.legSearchCache.set(cacheKey, { data: forwardOnly, timestamp: Date.now() });
          return forwardOnly;
        }
      }
    } catch (err: any) {
      winstonLogger.warn(`[SPLIT_SEARCH_DB] DB fallback error: ${from}->${to}: ${err.message}`);
    }

    winstonLogger.info(`[SPLIT_SEARCH_MISS] No verified data: ${from}->${to}`);
    return [];
  }

  /** Map raw API response to a basic Leg (used for direct trains) */
  private mapToLeg(raw: any, fromCode = '', toCode = '', avail?: any): Leg {
    // —— Compute duration from all available sources ——————————————————————————
    let durationMins = 0;

    // 1) Explicit numeric field
    if (raw.duration_mins > 0) durationMins = raw.duration_mins;
    else if (raw.durationMins > 0) durationMins = raw.durationMins;
    else if (typeof raw.duration === 'number' && raw.duration > 0) durationMins = raw.duration;
  ...
  private mapToRichLeg(
    ...
    let trainNo = normalizeTrainNumber(String(
      raw.train_number ||
      raw.train_no ||
      raw.trainNo ||
      raw.number ||
    ...
    if (!trainNo || trainNo === '00000') {
      winstonLogger.warn(`[SPLIT_INVALID_LEG] Missing trainNo for: ${raw.name}`);
      trainNo = "N/A";
    }
    ...
    if (!trainName && trainNo && trainNo !== 'N/A' && trainNo !== '00000') {
      const dbEntry = (dbTrains as any[]).find(
        (t: any) => String(t.trainNo) === trainNo || String(t.number) === trainNo
      );
      if (dbEntry) {
    ...
    if (raw.duration_mins > 0) apiDurationMins = raw.duration_mins;
    else if (raw.durationMins > 0) apiDurationMins = raw.durationMins;
    else if (typeof raw.duration === 'number' && raw.duration > 0) apiDurationMins = raw.duration;
    ...
    if (arrDay === 0) {
      if (apiDurationMins > 0) {
        ...
        const rawDurMins =
          parseInt(raw.duration_in_min || raw.durationInMinutes || raw.duration_mins || String(0)) || 0;
        if (rawDurMins > 0) {
    ...
    if (durationMins === 0 && apiDurationMins > 0) durationMins = apiDurationMins;
  ...
  private parseAvailability(avail: any) {
    ...
    if (!avail) {
      winstonLogger.debug(`[SPLIT_AVAIL] No availability payload — defaulting to CHECK_IRCTC`);
      return { status: 'CHECK_IRCTC', wlCount: 0 };
    }
    const status = (avail.current_status || avail.status || '').toUpperCase();
    if (!status) {
      winstonLogger.debug(`[SPLIT_AVAIL] Availability payload has no status field`, avail);
      return { status: 'CHECK_IRCTC', wlCount: 0 };
    }
    let wlCount = 0;
    const match = status.match(/WL\s*(\d+)/i);
    if (match) wlCount = parseInt(match[1]);
   ...
   */
  private calcLegDuration(dep: string, arr: string, depDay: number, arrDay: number): number {
    if (!dep || !arr) return 0;
   ...
   */
  private calculateWaitMinutes(arrival: string, departure: string, rollover: boolean): number {
    const arrMins = this.parseToMins(arrival);
    const depMins = this.parseToMins(departure);
    if (rollover) return (1440 - arrMins) + depMins;
   ...
   */
  private toEpochMs(baseDateStr: string, time: string, dayOffset: number = 1): number {
   ...
   */
  private inferDurationMins(dep: string, arr: string): number {
    if (!dep || !arr || dep === '--:--' || arr === '--:--') return 0;
    const depMins = this.parseToMins(dep);
    const arrMins = this.parseToMins(arr);
    if (arrMins > depMins) return arrMins - depMins;
    // Midnight rollover
   ...
   */
  private stationsMatch(code1: string, code2: string): boolean {
    if (!code1 || !code2) return true; // can't validate → allow
  ...
  private buildAiExplanation(
    l1: RichLeg, l2: RichLeg,
    from: string, hub: string, to: string,
    waitHours: number, riskLabel: string
  ): string {
    const trainA = l1.trainName || l1.trainNo || 'Train';
    const trainB = l2.trainName || l2.trainNo || 'Train';
    return (
      `Take ${trainA} from ${from} to ${hub} (arrives ${l1.arrival}), ` +
      `then board ${trainB} to ${to} (departs ${l2.departure}). ` +
      `${riskLabel} transfer — ${waitHours}h buffer at ${hub}.`
    );
  }

  private parseToMins(time: string): number {
    if (!time) return 0;
    const [h, m] = time.split(':').map(Number);
    return (h || 0) * 60 + (m || 0);
  }

  private incrementDate(dateStr: string, days: number): string {
  ...
  private shiftSplitDates(result: SplitEngineResult, targetDate: string, sourceDate: string): SplitEngineResult {
    if (!result || !result.split) return result;
    ...
    const shiftedSplits = result.split.map((s: any) => {
      const leg1 = s.leg1 ? {
        ...s.leg1,
   ...
   */
  private getMajorCorridorHubs(sourceCity: string, destinationCity: string): string[] {
    // Define major corridor hubs based on source and destination cities
    const corridorHubs: Record<string, Record<string, string[]>> = {
      mumbai: {
   ...
   */
  private getForcedMajorHubs(srcCode: string, dstCode: string, srcCity: string, dstCity: string): string[] {
    ...
    if (isMumbai && isNorth) return ['BRC', 'RTM', 'KOTA', 'GWL'];
    if (isMumbai && isGadag) return ['PUNE', 'SUR', 'UBL'];
    if (isMumbai && isEast) return ['BSL', 'NGP', 'ROU', 'TAT'];
   ...
   */
  private async findSplitsThroughHub(
    sourceCode: string,
    sourceName: string,
    hubCode: string,
    destCode: string,
    ...
    try {
      // Get hub name
      const hubName = (await stationService.getStationName(hubCode)) || hubCode;
    ...
    return splits;
      ...
      winstonLogger.error(`[SPLIT_ENGINE] Error in findSplitsThroughHub: ${error.message}`);
      return [];
    }
  }
}

export const splitJourneyEngine = new SplitJourneyEngine();
