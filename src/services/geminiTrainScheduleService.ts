...
export interface GeminiTrainSchedule {
  train_number: string;
  train_name: string;
  stations: GeminiStation[];
}
...
- Do NOT make up station data. Only return if you are confident.`;
    ...
    if (schedule.train_name && schedule.train_name.trim()) {
      try {
        ...
        const { error: e1 } = await supabase
          .from('trains')
          .upsert(
            { number: trainNo, name: schedule.train_name.trim() },
            { onConflict: 'number' }
          );
        if (e1) {
          ...
          try {
            await supabase
              .from('trains')
              .upsert(
                { Train_No: trainNo, Train_Name: schedule.train_name.trim() },
                { onConflict: 'Train_No' }
              );
          } catch { /* best-effort */ }
        }
  ...
  async getAndSave(trainNo: string): Promise<{
    ...
    const parseToMins = (t: string): number => {
      if (!t || t === '--:--') return -1;
...
export const geminiTrainScheduleService = new GeminiTrainScheduleService();
