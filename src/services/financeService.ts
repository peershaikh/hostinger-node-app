...
/**
 * Finance Service: Trayago Personal Wealth & Expense Module
 * Handles daily expense tracking, investment predictions, and financial health scoring.
 */
...
export class FinanceService {
  ...
  private getCurrentUserId(): string {
    ...
    throw new Error(
      'User ID not available. Please pass userId from controller or implement auth middleware.'
    );
  }
  ...
  async logExpense(
    ...
    try {
      ...
      winstonLogger.info(`[FINANCE] Expense logged: ₹${amount} - ${category} (User: ${userId})`);
      ...
      winstonLogger.error(`[FINANCE] Failed to log expense: ${e.message}`);
  ...
  async getInvestmentPredictions() {
    ...
    try {
      ...
      const predictions: InvestmentPrediction[] = [
        {
      ...
      winstonLogger.error(`[FINANCE] Prediction failed: ${e.message}`);
      return {
        predictions: [],
        health_score: 40,
        timestamp: new Date().toISOString(),
  ...
  async getFinancialHealthScore(): Promise<FinancialHealthScore> {
    ...
    try {
      ...
      winstonLogger.error(`[FINANCE] Health score failed: ${e.message}`);
      return {
        score: 35,
        status: 'MODERATE',
  ...
  private calculateBasicHealthScore(expenses: any[]): FinancialHealthScore {
    // (Same implementation as before - kept clean)
    const totalSpent = expenses.reduce((sum, e) => sum + Number(e.amount), 0);
    const travelExpenses = expenses
      .filter((e) =>
        ['TRAVEL', 'TRANSPORT', 'HOTEL', 'FLIGHT', 'TRAIN', 'RAIL'].includes(e.category)
      )
    ...
    const finalScore = Math.round(
      spendingScore * 0.35 + savingsPotential * 0.35 + consistencyScore * 0.30
    );
    ...
    if (travelRatio > 40) insights.push('Travel expenses are high — consider budget planning.');
    if (avgDaily > 800) insights.push('Daily spending appears elevated.');
