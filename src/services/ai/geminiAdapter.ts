...
export class GeminiAdapter implements AiProvider {
  ...
  private getEndpointUrl(): string {
    const model = aiConfig.gemini.model || 'gemini-2.5-flash';
  ...
  private async executeGeminiCall(
    ...
    const url = `${this.getEndpointUrl()}?key=${apiKey}`;
    ...
    try {
      winstonLogger.info(`[AI_CALL] [GEMINI_ACTIVE] Model: ${aiConfig.gemini.model}`);
