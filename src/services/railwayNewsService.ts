...
import { CanonicalNewsArticle, SourceHealth } from './news/newsTypes';
import { newsSourceRegistry } from './news/newsSourceRegistry';
import { newsIngestionEngine } from './news/newsIngestionEngine';
...
function readLocalNewsFallback(): NewsArticle[] {
  try {
    ...
    winstonLogger.warn('[NEWS_LOCAL_FALLBACK_READ_FAIL]', { error: err.message });
  }
  return [];
}

function writeLocalNewsFallback(articles: NewsArticle[]): void {
  try {
    const dir = path.dirname(LOCAL_FALLBACK_FILE);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    safeWriteFileSync(LOCAL_FALLBACK_FILE, JSON.stringify(articles, null, 2));
  } catch (err: any) {
    winstonLogger.warn('[NEWS_LOCAL_FALLBACK_WRITE_FAIL]', { error: err.message });
  }
}
...
export const railwayNewsService = {
   ...
   */
  getSourceHealthSummary: (): SourceHealth[] => {
    return newsSourceRegistry.getHealthSummary();
  },

  /**
   * Triggers push notification for high-priority rail updates.
   */
  triggerPushAlert: async (article: NewsArticle): Promise<void> => {
    ...
    if (alertCategories.includes(article.category)) {
      ...
      try {
        ...
        winstonLogger.warn('[PUSH_ALERT_FAIL]', { error: err.message });
      }
    }
  },
};