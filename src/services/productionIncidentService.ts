...
import { ProductionIncidentReport, TelemetryIncident, GeneratedEngineeringTask, IncidentSeverity } from '../types/productionIncidents';

export class ProductionIncidentService {
  private cachedReport: ProductionIncidentReport | null = null;
  ...
  public async getProductionIncidentReport(): Promise<ProductionIncidentReport> {
    ...
    const incidentsMap = new Map<string, TelemetryIncident>();
    const tasks: GeneratedEngineeringTask[] = [];
    ...
    try {
      ...
      winstonLogger.warn(`[INCIDENT_ENGINE] Supabase query telemetry error: ${err.message}`);
    }
    ...
    const report: ProductionIncidentReport = {
      generated_at: nowIso,
      execution_time_ms: Date.now() - startTime,
      metrics: {
...
export const productionIncidentService = new ProductionIncidentService();
