...
async function loadTrainScheduleContext(
  ...
  const isDailyAuthoritativeFromDb =
    !runningDaysFromHint &&
    !isSpecialTrainNumber(tNo) &&
  ...
  if (isDailyAuthoritativeFromDb) {
    runningDaysAuthoritative = true;
    winstonLogger.debug(
      `[STATION_RESOLVER] RD_DB_AUTH for ${tNo}: non-special train with ${dbStops.length} DB schedule stops — treating "Daily" as authoritative (PHASE_087I)`
    );
  }
