...
export class EmailService {
  async sendOtpEmail(toEmail: string, otpCode: string): Promise<boolean> {
    try {
      ...
      if (error) {
        winstonLogger.error(`[EMAIL_ERROR] Failed to send OTP to ${toEmail}`, error);
        throw new Error(error.message);
      }

      winstonLogger.info(`[EMAIL_SUCCESS] OTP sent to ${toEmail}`);
      return true;
    } catch (err: any) {
      winstonLogger.error(`[EMAIL_EXCEPTION] Exception while sending OTP to ${toEmail}`, err);
  ...
  async sendPasswordResetEmail(toEmail: string, otpCode: string): Promise<boolean> {
    try {
      ...
      if (error) {
        winstonLogger.error(`[EMAIL_ERROR] Failed to send password reset OTP to ${toEmail}`, error);
        throw new Error(error.message);
      }

      winstonLogger.info(`[EMAIL_SUCCESS] Password reset OTP sent to ${toEmail}`);
      return true;
    } catch (err: any) {
      winstonLogger.error(`[EMAIL_EXCEPTION] Exception while sending password reset OTP to ${toEmail}`, err);
  ...
  async sendAlertEmail(toEmail: string, alertTitle: string, alertMessage: string): Promise<boolean> {
    try {
      ...
      if (error) {
        winstonLogger.error(`[EMAIL_ERROR] Failed to send alert to ${toEmail}`, error);
        throw new Error(error.message);
      }
      return true;
    } catch (err: any) {
  ...
  async sendHealthReportEmail(toEmail: string, subject: string, htmlContent: string): Promise<boolean> {
    try {
      ...
      if (error) {
        winstonLogger.error(`[EMAIL_ERROR] Failed to send health report to ${toEmail}`, error);
        throw new Error(error.message);
      }
      return true;
    } catch (err: any) {
