...
export interface SearchMetricsContext {
  route: string;
  availCalls: number;
  findSegmentSplitsCalls: number;
  duplicateSplitPrevented: number;
  startedAt: number;
}

let activeSearchContext: SearchMetricsContext | null = null;
...
function emit(metric: string, payload: Record<string, unknown>): void {
  if (!featureFlags.knowledgeMetrics) return;
  try {
...
export const knowledgeMetricsService = {
  beginSearchContext(source: string, destination: string): void {
    activeSearchContext = {
  ...
  recordAvailabilityProviderCall(): void {
    lifetime.availabilityProviderCalls++;
    if (activeSearchContext) activeSearchContext.availCalls++;
    emit('availability_provider_call', { total: lifetime.availabilityProviderCalls });
  },

  recordFindSegmentSplitsInvocation(): void {
    lifetime.findSegmentSplitsCalls++;
    if (activeSearchContext) activeSearchContext.findSegmentSplitsCalls++;
    emit('find_segment_splits', { total: lifetime.findSegmentSplitsCalls });
  },

  recordDuplicateSplitPrevented(): void {
    lifetime.duplicateSplitPrevented++;
    if (activeSearchContext) activeSearchContext.duplicateSplitPrevented++;
    emit('duplicate_split_prevented', { total: lifetime.duplicateSplitPrevented });
  },

  recordCacheHit(layer: 'canonical' | 'legacy'): void {
    lifetime.cacheHits++;
  ...
  recordSearchLatency(latencyMs: number, route?: string): void {
    lifetime.searchLatencySamples++;
    lifetime.searchLatencyTotalMs += latencyMs;
    emit('search_latency', { route, search_latency_ms: latencyMs });
  },

  recordRescueLatency(latencyMs: number, trainNo?: string): void {
  ...
  recordShadowCompare(match: boolean, matchRate: number): void {
    lifetime.shadowCompareSamples++;
    if (match) lifetime.shadowCompareMatches++;
  ...
  recordB1DualRead(payload: {
    ...
    catalog_runtime_diff: { runtime_only: string[]; catalog_only: string[] };
    ...
    if (payload.catalog_hit) lifetime.catalogHit++;
    if (payload.catalog_miss) lifetime.catalogMiss++;
    if (payload.hydration_success) lifetime.hydrationSuccess++;
    if (payload.hydration_failure) lifetime.hydrationFailure++;
