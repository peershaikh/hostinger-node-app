...
export class DailyHealthReportJob {
  start() {
    winstonLogger.info('[DAILY_HEALTH_REPORT] Scheduled for 00:00 every day.');
  ...
  private async generateAndSend() {
    ...
    await emailService.sendHealthReportEmail(adminEmail, `Trayago Daily Health: ${status.system_health}`, html);
    
    winstonLogger.info(`[DAILY_HEALTH_REPORT] Sent report to ${adminEmail}.`);
  }
}
