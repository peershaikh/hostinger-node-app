...
export const getDistance = (a: Station, b: Station): number => {
  const R = 6371;
...
export const isForwardStation = (
  source: string,
  destination: string,
  via: string,
  maxDetourRatio: number = 1.8
): boolean => {
  const src  = STATIONS[source];
  const dest = STATIONS[destination];
  const mid  = STATIONS[via];

  if (!src || !dest || !mid) return true; // allow if coords not known

  const direct = getDistance(src, dest);
  const split  = getDistance(src, mid) + getDistance(mid, dest);
...
export const isValidDirectionalFlow = (
  source: string,
  destination: string,
  via: string,
  marginKm: number = 50
  ...
  const directDist = getDistance(src, dest);
  const hubToDestDist = getDistance(mid, dest);
...
export const getValidViaStations = (
  source: string,
  destination: string,
  candidates?: string[]
): string[] => {
  const pool = candidates ?? Object.keys(STATIONS);
  return pool.filter(
    (stn) => stn !== source && stn !== destination && isForwardStation(source, destination, stn) && isValidDirectionalFlow(source, destination, stn)
  );
};// 📊 Sort via stations by distance from source (closest first)
// Returns top N ranked candidates for multi-via exploration.
export const sortViaByDistance = (
  source: string,
  viaStations: string[],
  topN: number = 50
): string[] => {
  const src = STATIONS[source];
  if (!src) return viaStations.slice(0, topN);

  return viaStations
    .map((code) => {
      const stn = STATIONS[code];
      const dist = stn ? getDistance(src, stn) : Infinity;
      return { code, dist };
    })
...
export const sortViaByDetourScore = (
  source: string,
  destination: string,
  viaStations: string[],
  topN: number = 50
  ...
  const src = STATIONS[source];
  const dest = STATIONS[destination];
  if (!src || !dest) return sortViaByDistance(source, viaStations, topN);

  const directDist = getDistance(src, dest);

  return viaStations
    .map((code) => {
      const mid = STATIONS[code];
      if (!mid) return { code, score: Infinity };
      
      const srcToMid = getDistance(src, mid);
      const midToDest = getDistance(mid, dest);
...
// ─────────────────────────────────────────────────────────────────────────────
export const HARD_DETOUR_THRESHOLD = 2.5;
...
export const getDetourRatio = (
  ...
  const splitDist = getDistance(src, mid) + getDistance(mid, dest);
 ...
 */
export const isHardDetour = (
  source: string,
  hub: string,
  destination: string
): boolean => {
  const ratio = getDetourRatio(source, hub, destination);
  if (ratio === null) return false; // coords missing → allow
  return ratio > HARD_DETOUR_THRESHOLD;
};
