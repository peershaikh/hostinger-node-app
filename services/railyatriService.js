...
exports.railyatriService = exports.RailyatriService = void 0;
...
class RailyatriService {
    async getTrainStatus(trainNo, date) {
        ...
        if (cached) {
            logger_1.winstonLogger.info(`[RAILYATRI_CACHE_HIT] ${trainNo}`);
            return cached;
        }
        try {
            logger_1.winstonLogger.info(`[API_FALLBACK_1] RailYatri → Fetching live status for ${trainNo}`);
            const keys = await providerConfigService_1.providerConfigService.getKeysFor('RAILYATRI');
            if (keys.length === 0) {
                logger_1.winstonLogger.warn(`[PROVIDER_MISSING_KEY] RailYatri: No API key found in configuration.`);
                return null;
            }
            const apiKey = keys[0];
            ...
            const normalizedStations = Array.isArray(rawStations) ? rawStations.map((s) => ({
                ...s,
            ...
            logger_1.winstonLogger.info(`[RAILYATRI_SUCCESS] ${trainNo} @ ${normalized.current_station || 'Moving'}`);
            return normalized;
        }
        catch (err) {
            const isUnauthorized = err.response?.status === 401 || err.response?.status === 403;
            if (isUnauthorized) {
                logger_1.winstonLogger.warn(`[PROVIDER_INVALID_KEY] RailYatri: Request failed with status code ${err.response?.status}`);
            }
            else {
                logger_1.winstonLogger.warn(`[RAILYATRI_FAIL] ${trainNo}: ${err.message}`);
            }
            return null;
        }
    }
...
exports.RailyatriService = RailyatriService;
exports.railyatriService = new RailyatriService();
