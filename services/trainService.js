...
const SOURCES = ['IRCTC', 'RAILRADAR', 'RAPIDAPI', 'DB'];
...
class TrainService {
    async getTrainData(source, destination, date) {
        ...
        if (Array.isArray(cachedSearch) && cachedSearch.length > 0) {
            ...
            logger_1.winstonLogger.info(`[TRAIN_SERVICE] search cache hit directCount=${direct.length}`);
            return {
                direct,
        ...
        if (!directResults.length) {
            ...
            if (anyProviderAttempted && !anyProviderSucceeded) {
                logger_1.winstonLogger.warn(`[PROVIDER_ALL_FAILED] All live providers exhausted for ${sCode}→${dCode}. ` +
                    `Attempted: [${providerAttempted.join(', ')}]. Falling back to DB.`);
            }
            else if (!anyProviderAttempted) {
                logger_1.winstonLogger.warn(`[NO_PROVIDER_AVAILABLE] No live providers enabled/configured for ${sCode}→${dCode}. ` +
                    `Falling back to DB.`);
            }
            ...
            try {
                ...
                if (Array.isArray(dbTrains)) {
                    ...
                    for (const dbT of dbTrains) {
                        ...
                        if (tNo && !liveTrainNos.has(tNo)) {
                            logger_1.winstonLogger.info(`[CANCELLED_TRAIN_DETECTED] ${tNo} found in DB but missing from live provider (${sourceUsed})`);
                ...
                logger_1.winstonLogger.error(`[CANCELLED_CHECK_FAILED] ${e.message}`);
            }
        }
        const direct = this.normalizeAndDeduplicate(directResults || [], searchDate, sCode, dCode);
        if (direct.length > 0 && sourceUsed !== 'DB') {
            cacheService_1.cacheService.cacheSearch(sCode, dCode, searchDate, directResults);
        }
        logger_1.winstonLogger.info(`[TRAIN_SERVICE] source=${sourceUsed} directCount=${direct.length}`);
        ...
        if (unnamedTrains.length > 0) {
            ...
            await Promise.allSettled(enrichmentTargets.map(async (train) => {
                try {
                    const dbName = await dbService_1.dbService.dbLookupTrainName(train.trainNo);
                    if (dbName) {
                        train.name = dbName;
                        train.train_name = dbName;
                        logger_1.winstonLogger.debug(`[NAME_ENRICH] ${train.trainNo} → "${dbName}" (DB)`);
                    }
                    else {
                        ...
                        logger_1.winstonLogger.debug(`[NAME_ENRICH] ${train.trainNo} → synthetic fallback`);
                    }
                }
                catch (e) {
                    // Non-fatal: leave name as null; frontend has its own fallback
                    logger_1.winstonLogger.warn(`[NAME_ENRICH_FAIL] ${train.trainNo}: ${e.message}`);
                }
            }));
        }
    ...
    validateTrain(train, ss, ds, searchDate) {
        ...
        if (train.isHistorical || train.historical || train.is_historical) {
            if (process.env.NODE_ENV !== 'production')
                console.log(`[TRAIN_REJECTED_HISTORICAL] Train ${train.number || train.trainNo} is historical`);
        ...
        if (status === 'CANCELLED' || status.includes('SUSPENDED') || status.includes('CANCEL')) {
            if (process.env.NODE_ENV !== 'production')
                console.log(`[TRAIN_REJECTED_CANCELLED] Train ${train.number || train.trainNo} is cancelled/suspended`);
        ...
        if (!rawRunningDays) {
            if (process.env.NODE_ENV !== 'production')
                console.log(`[TRAIN_ALLOWED_METADATA_MISSING] Train ${train.number || train.trainNo} missing running days data, defaulting to true`);
            return { isValid: true, reason: 'METADATA_MISSING' };
        }
        const binary = (0, dayUtils_1.normalizeRunningDays)(rawRunningDays);
        // Only reject if explicit mismatch confirmed
        if (binary && !(0, dayUtils_1.isDayActive)(binary, searchDate)) {
            if (process.env.NODE_ENV !== 'production')
                console.log(`[TRAIN_REJECTED_NOT_RUNNING] Train ${train.number || train.trainNo} does not run on ${searchDate}`);
    ...
    normalizeTrain(dt) {
        ...
        return {
            ...
            from: safe(dt.fromStationCode || dt.source || dt.from, "N/A"),
            to: safe(dt.toStationCode || dt.destination || dt.to, "N/A"),
            departure: safe(dt.departureTime || dt.departure, "--:--"),
            arrival: safe(dt.arrivalTime || dt.arrival, "--:--"),
            duration: typeof dt.duration_mins === "number"
                ? dt.duration_mins
                : 0,
    ...
    async searchDirect(source, destination, searchDate) {
        ...
        try {
            let [sourceRes, destRes] = await Promise.all([
                supabase_1.supabase.from('train_schedule').select('*').eq('Station_Code', s).order('SN'),
                supabase_1.supabase.from('train_schedule').select('*').eq('Station_Code', d).order('SN'),
            ]);
            if (sourceRes.data?.length === 0 && s === 'CSMT') {
                sourceRes = await supabase_1.supabase.from('train_schedule').select('*').eq('Station_Code', 'CSTM').order('SN');
            }
            if (destRes.data?.length === 0 && d === 'CSMT') {
                destRes = await supabase_1.supabase.from('train_schedule').select('*').eq('Station_Code', 'CSTM').order('SN');
            }
            const sources = (sourceRes.data || []).map((row) => ({
                ...row,
                Train_No: (0, availabilityCacheKeys_1.normalizeTrainNumber)(row.Train_No.toString())
            }));
            const destinations = (destRes.data || []).map((row) => ({
                ...row,
                Train_No: (0, availabilityCacheKeys_1.normalizeTrainNumber)(row.Train_No.toString())
            }));
            ...
            const trainNumbers = [
                ...new Set([
                    ...sources.map((row) => row.Train_No),
                    ...destinations.map((row) => row.Train_No),
                ]),
            ];
            const { data: metaData } = await supabase_1.supabase
                .from('trains')
                .select('number, name, type, running_days')
                .in('number', trainNumbers);
            const metaMap = new Map((metaData || []).map((row) => [row.number, row]));
            // Dynamically compute day numbers for stations because train_schedule lacks Day_number column
            const { data: allSchedules } = await supabase_1.supabase
                .from('train_schedule')
            ...
            if (allSchedules && allSchedules.length > 0) {
                ...
                for (const row of allSchedules) {
                    const tNo = (0, availabilityCacheKeys_1.normalizeTrainNumber)(row.Train_No.toString());
                    if (!trainGroups[tNo])
                        trainGroups[tNo] = [];
            ...
            for (const ss of sources) {
                const potentialDests = destinations.filter((ds) => ds.Train_No.toString() === ss.Train_No.toString());
                for (const ds of potentialDests) {
                    ...
                    const durationMins = rankingService_1.rankingService.calculateCorrectDuration?.(ss.Departure_Time, ds.Arrival_time, depDay, arrDay) || 0;
                    matchingTrains.push({
                        id: `db-${ss.Train_No}`,
                        ...
                        total_journey_time: rankingService_1.rankingService.formatDuration?.(durationMins) || '',
            ...
            logger_1.winstonLogger.error(`[TRAIN_DIRECT] Failed: ${err.message}`);
            return [];
        }
    }
    async searchAdvanced(source, destination, date, options) {
        const searchDate = date || new Date().toISOString().split('T')[0];
        const includeSplit = typeof options === 'boolean'
            ? options
            : options?.includeSplit === true;
        logger_1.winstonLogger.info(`[TRAIN_SEARCH] ${source} -> ${destination} | ${searchDate}`);
        ...
        if (includeSplit) {
            try {
                const { splitJourneyEngine } = await Promise.resolve().then(() => __importStar(require('./splitJourneyEngine')));
                let splitTimer;
                const splitTimeout = new Promise((resolve) => {
                    splitTimer = setTimeout(() => {
                        logger_1.winstonLogger.warn('[TRAIN_SEARCH] Split search timed out after 28s');
                        resolve({ split: [], smart_routes: [], message: 'Search timeout' });
                    }, 28000);
                });
                const splitSearchPromise = splitJourneyEngine.findCombinedRoutes(source.toUpperCase(), destination.toUpperCase(), searchDate, normalizedDirect, undefined, { classType: '3A', quota: 'GN' });
                ...
                logger_1.winstonLogger.info(`[TRAIN_SEARCH] Split results: ${finalSplits.length}`);
            }
            catch (error) {
    ...
    normalizeAndDeduplicate(trains, date, sourceCode, destinationCode) {
        const seen = new Set();
        const unique = [];
    ...
    stationMatches(actual, expected) {
    ...
    normalizeSingleTrain(t, date) {
        ...
        return {
            ...
            total_journey_time: t.travel_time ||
                t.total_journey_time ||
                rankingService_1.rankingService.formatDuration?.(t.duration_mins || 0) ||
                '',
    ...
    _persistSearchAsync(source, destination, date, ranked, apiSuccess) {
        setImmediate(async () => {
            try {
                if (ranked.length > 0) {
                    await dbService_1.dbService.saveSearchToDB({
                        source,
                        destination,
                        date,
                ...
                await dbService_1.dbService.saveSearchPopularity({ source, destination });
            }
            catch (e) {
                logger_1.winstonLogger.debug(`[PERSIST] Background save failed: ${e.message}`);
            }
        });
    }
    generateTwitterUrl(trainNo, coachOrDate = 'N/A', issue = 'Journey Issue') {
     ...
     */
    async getTrainMetadata(trainNo) {
        try {
     ...
     */
    async getCoachComposition(trainNo) {
        try {
            const { dbService } = await Promise.resolve().then(() => __importStar(require('./dbService')));
            const supabase = dbService.supabase;
            if (!supabase)
