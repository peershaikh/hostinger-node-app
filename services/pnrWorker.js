...
class PnrWorker {
    ...
    async updatePnrStatus(record) {
        try {
            ...
            if (cached) {
                logger_1.winstonLogger.debug(`[PNR_WORKER] Cache hit for ${record.pnr_number}`);
                return false;
            }
            ...
            if (success) {
                ...
                cacheService_1.cacheService.set(cacheKey, true, 10);
                if (hasChanged) {
                    ...
                    await alertService_1.alertService.triggerWaitlistAlert(record.session_id, record.pnr_number, record.current_status, newStatus);
                }
                else {
                ...
                if (chartJustPrepared) {
                    ...
                    await alertService_1.alertService.triggerChartPreparedAlert(record.session_id, record.pnr_number, newChartStatus, cnfCount, totalPassengers);
                }
                // ──────────────────────────────────────────────────────────────────────
            }
            return hasChanged;
