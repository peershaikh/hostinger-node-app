...
exports.adminIntelligenceV2Service = exports.AdminIntelligenceV2Service = void 0;
const supabase_1 = require("../config/supabase");
const logger_1 = require("../middleware/logger");
const metricsService_1 = require("./metricsService");
class AdminIntelligenceV2Service {
    constructor() {
    ...
    async getIntelligenceReport() {
        ...
        try {
            if ((0, supabase_1.isSupabaseConfigured)()) {
                ...
                // Feedback evidence
                const { count: totalFb } = await supabase_1.supabase.from('user_feedback').select('*', { count: 'exact', head: true });
                const { count: unresFb } = await supabase_1.supabase.from('user_feedback').select('*', { count: 'exact', head: true }).eq('status', 'PENDING');
                ...
                // Users count
                const { count: uTotal } = await supabase_1.supabase.from('users').select('*', { count: 'exact', head: true });
                const { count: uNew } = await supabase_1.supabase.from('users').select('*', { count: 'exact', head: true }).gte('created_at', twentyFourHoursAgo);
                totalUsers = uTotal || 0;
                newUsers24h = uNew || 0;
            }
        }
        catch (err) {
            logger_1.winstonLogger.warn(`[INTELLIGENCE_V2] Supabase telemetry query error: ${err.message}`);
        }
        ...
        let sysMetrics = {};
        try {
            sysMetrics = await metricsService_1.metricsService.getSystemStatus();
        }
        catch (e) {
            logger_1.winstonLogger.warn(`[INTELLIGENCE_V2] Metrics fallback: ${e.message}`);
        }
        const report = {
            generated_at: nowIso,
            execution_time_ms: Date.now() - startTime,
            ...
            report_sections: {
                ...
                database: { status: (0, supabase_1.isSupabaseConfigured)() ? 'CONNECTED' : 'DISCONNECTED' },
...
exports.AdminIntelligenceV2Service = AdminIntelligenceV2Service;
exports.adminIntelligenceV2Service = new AdminIntelligenceV2Service();
