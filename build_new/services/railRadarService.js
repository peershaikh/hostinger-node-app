...
class RailRadarService {
    async getTrainStatus(trainNo) {
        ...
        if (cached) {
            logger_1.winstonLogger.info(`[RAILRADAR_CACHE_HIT] ${trainNo}`);
            return cached;
        }
        ...
        try {
            logger_1.winstonLogger.info(`[API_FALLBACK_1] RailRadar → Fetching live status for ${trainNo}`);
            const apiKey = keys[0];
            ...
            logger_1.winstonLogger.info(`[RAILRADAR_SUCCESS] ${trainNo} @ ${normalized.current_station || 'Moving'}`);
            return normalized;
        }
        catch (err) {
            const isUnauthorized = err.response?.status === 401;
            if (isUnauthorized) {
                logger_1.winstonLogger.warn(`[PROVIDER_INVALID_KEY] RailRadar: Request failed with status code 401`);
            }
            else {
                logger_1.winstonLogger.warn(`[RAILRADAR_FAIL] ${trainNo}: ${err.message}`);
            }
            return null;
        }
    }
    ...
    async search(from, to, date) {
        ...
        logger_1.winstonLogger.warn(`[PROVIDER_STUB_INACTIVE] RailRadar.search(${from}→${to}) called but ` +
    ...
    async getPNRStatus(pnr) {
        ...
        if (keys.length === 0)
            return null;
        logger_1.winstonLogger.info(`[RAILRADAR_PNR] ${pnr} (Not supported yet)`);
        return null;
    }
    /** Optional: Availability support (if available) */
    async getAvailability(trainNo, from, to, date, classType, quota) {
        const keys = await providerConfigService_1.providerConfigService.getKeysFor('RAILRADAR');
        if (keys.length === 0)
            return null;
        logger_1.winstonLogger.info(`[RAILRADAR_AVAIL] ${trainNo} from=${from} to=${to} (Not supported yet)`);
        return null;
    }
    getStatus() {
        return {
