...
class ProviderConfigService {
    ...
    async isProviderEnabled(providerName) {
        ...
        const cachedResult = cacheService_1.cacheService.get(enabledCacheKey);
        if (cachedResult)
            return cachedResult;
        const _cache = (r) => {
            cacheService_1.cacheService.set(enabledCacheKey, r, 10); // 10s TTL
            return r;
        };
        ...
        if (useDb) {
            try {
                ...
                logger_1.winstonLogger.warn(`[isProviderEnabled] DB check failed for ${nameUpper}: ${err.message}. Falling back to env.`);
            }
        }
    ...
    decryptKey(encryptedText) {
        try {
            ...
            const decipher = crypto_1.default.createDecipheriv(ENCRYPTION_ALGORITHM, this.getEncryptionKey(), iv);
    ...
    encryptKey(rawKey) {
        try {
            ...
            const cipher = crypto_1.default.createCipheriv(ENCRYPTION_ALGORITHM, this.getEncryptionKey(), iv);
            ...
            logger_1.winstonLogger.error(`[ProviderConfigService] Failed to encrypt API key: ${error.message}`);
            throw error;
        }
    }
    /**
     ...
     */
    async getKeysFor(providerName) {
        const useDb = process.env.USE_DB_PROVIDERS === 'true';
        const now = Date.now();
        if (!useDb) {
            logger_1.winstonLogger.debug(`[ProviderConfigService] USE_DB_PROVIDERS is false. Using .env fallback for ${providerName}`);
        ...
        if ((this.consecutiveFailures[providerName] || 0) >= 3) {
            if (now < (this.circuitOpenUntil[providerName] || 0)) {
                ...
                logger_1.winstonLogger.info(`[CIRCUIT_BREAKER_HALF_OPEN] Probing DB recovery for ${providerName}`);
                this.telemetry.circuit_half_open_count++;
            }
        }
        const cacheKey = `provider_keys_${providerName}`;
        const cachedKeys = cacheService_1.cacheService.get(cacheKey);
        ...
        try {
            ...
            if (!data || data.length === 0) {
                logger_1.winstonLogger.warn(`[ProviderConfigService] No active DB keys found for ${providerName}. Falling back to .env.`);
                return this.getEnvFallback(providerName);
            }
            const activeRows = data.filter((row) => row.enabled === true);
            if (activeRows.length === 0) {
                logger_1.winstonLogger.info(`[PROVIDER_DISABLED] ${providerName} is explicitly disabled in DB. Skipping fallback.`);
                return [];
            }
            const keys = [];
            for (const row of activeRows) {
                try {
                    ...
                    if (row.api_key && row.api_key.includes(':')) {
                        ...
                        logger_1.winstonLogger.warn(`[ProviderConfigService] Found unencrypted plain-text key for ${providerName}. Please rotate immediately!`);
            ...
            if (keys.length === 0) {
                logger_1.winstonLogger.error(`[ProviderConfigService] Decryption failed for all DB keys of ${providerName}. Falling back to .env.`);
                return this.getEnvFallback(providerName);
            }
            logger_1.winstonLogger.info(`[PROVIDER_DB_OVERRIDE] Loaded active keys from DB for provider: ${providerName}`);
            // Cache the successfully decrypted keys
            cacheService_1.cacheService.set(cacheKey, keys, this.CACHE_TTL);
            // Reset circuit breaker on success
            if ((this.consecutiveFailures[providerName] || 0) >= 3) {
                logger_1.winstonLogger.info(`[CIRCUIT_BREAKER_RECOVERED] DB connection restored for ${providerName}`);
            ...
            if (this.consecutiveFailures[providerName] === 3) {
                ...
                logger_1.winstonLogger.error(`[CIRCUIT_BREAKER_OPEN] DB unavailable. Bypassing for 3 minutes.`);
            }
            logger_1.winstonLogger.error(`[ProviderConfigService] DB fetch failed for ${providerName}: ${error.message}. Falling back to .env.`);
            return this.getEnvFallback(providerName);
        }
    }
    /**
