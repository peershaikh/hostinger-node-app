...
class LlmService {
    ...
    `;
      ...
      5. YOUR PROBABILITY OUTPUT MUST STAY WITHIN ±15 POINTS OF THE HEURISTIC CEILING FOR THIS WL TYPE AND POSITION.
         Heuristic ceiling for ${wlType}/${wlPos}: approximately ${(() => {
                if (wlType === 'GNWL')
                    return wlPos <= 10 ? 88 : wlPos <= 20 ? 78 : wlPos <= 35 ? 62 : wlPos <= 60 ? 42 : 22;
                if (wlType === 'TQWL')
                    return wlPos <= 3 ? 22 : wlPos <= 8 ? 12 : 6;
                if (wlType === 'RLWL')
                    return wlPos <= 8 ? 65 : wlPos <= 18 ? 45 : wlPos <= 30 ? 28 : 15;
                if (wlType === 'PQWL')
                    return wlPos <= 5 ? 55 : wlPos <= 10 ? 38 : wlPos <= 20 ? 22 : 12;
    ...
    async categorizeFeedback(feedbackText, metadata = {}) {
        if (!this.GEMINI_KEY) {
            logger_1.winstonLogger.warn('[LLM] categorizeFeedback skipped — GEMINI_API_KEY not set');
            return null;
        }
        const prompt = `
  ...
  "suggestedAction": "One specific technical action for the dev team (max 15 words)"
    ...
    async cleanAvailabilityData(rawAvailString) {
        if (!rawAvailString || rawAvailString.trim() === '')
            return [];
        if (!this.GEMINI_KEY) {
            logger_1.winstonLogger.warn("[GEMINI] Key missing, falling back to basic parsing");
    ...
    Return ONLY a JSON array, e.g., [{"class": "3A", "status": "AVAILABLE", "count": 45}].`;
        try {
            logger_1.winstonLogger.info(`[AI_CALL] [GEMINI_ACTIVE] Cleaning availability data`);
            ...
            logger_1.winstonLogger.error("[GEMINI] Parsing failed");
            return [];
        }
    }
    async callAi(prompt, json = false) {
        ...
        try {
            logger_1.winstonLogger.info(`[AI_CALL] [GEMINI_ACTIVE] Prompting Gemini 2.5 Flash model`);
            ...
            logger_1.winstonLogger.warn(`[AI CALL ERROR]: ${err.message}`);
            throw err;
        }
    }
}
