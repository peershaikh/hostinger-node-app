...
exports.JourneyOrchestrator = exports.journeyOrchestrator = void 0;
const featureFlags_1 = require("../config/featureFlags");
const rescueIntelligence_1 = require("../rescue/rescueIntelligence");
// ─── Constants ────────────────────────────────────────────────────────────────
 ...
 */
function normalizeLowerBetter(score, scale = LOWER_BETTER_SCALE) {
    if (score <= 0)
        return 1.0;
 ...
 */
function normalizeHigherBetter(score, max) {
    if (max <= 0)
        return 0;
 ...
 */
function availabilityToConfidence(status) {
    if (!status)
        return 0.1;
...
// ─── Signal Extraction ────────────────────────────────────────────────────────
function extractSearchSignals(search) {
    if (!search)
        return [];
    const signals = [];
...
function extractSplitSignals(split) {
    if (!split)
        return [];
    const signals = [];
...
function extractRescueSignals(rescue) {
...
function extractPnrSignals(pnr, prediction) {
    if (!pnr)
        return [];
    const signals = [];
...
function extractLiveSignals(live) {
    if (!live)
        return [];
    const signals = [];
...
function extractBookingSignals(decision) {
    if (!decision)
        return [];
    const signals = [];
...
function makeCandidateId(type, trainNo, hub) {
    return `${type}:${trainNo}${hub ? `:${hub}` : ''}`;
}
function normalizeDirectCandidates(search, split, ctx) {
    const candidates = [];
    // From search result
    ...
    for (const train of searchDirect) {
        ...
        const scoreDirection = 'LOWER_BETTER';
    ...
    for (const leg of splitDirect) {
        ...
        if (candidates.some(c => c.trainNo === trainNo))
            continue;
        const availStatus = leg?.availability?.status;
        const availConf = availabilityToConfidence(availStatus);
        const rawScore = leg?.score ?? 0;
        const normalizedScore = normalizeLowerBetter(rawScore);
        const combinedConf = (availConf * 0.6) + (normalizedScore * 0.4);
        const components = [
            {
        ...
        candidates.push({
            ...
            destination: ctx.destination,
            legs: [leg],
            totalDuration: leg?.durationMins || leg?.duration_mins,
            travelDate: ctx.date,
            rawScore,
...
function normalizeSplitCandidates(split, ctx) {
    if (!split)
        return [];
    const candidates = [];
    const splits = split.split || split.splits || split.smart_routes || [];
...
function normalizeRescueCandidates(rescue, split, ctx) {
    const candidates = [];
...
// ─── Health Assessment ────────────────────────────────────────────────────────
function assessHealth(live, pnr, prediction) {
    const signals = [];
    let delayMinutes = 0;
    let isRunning = false;
    let status = 'UNKNOWN';
    let pnrStatus = 'UNKNOWN';
    let pnrProbability;
    // Live health
    if (live) {
    ...
    if (pnr) {
        ...
        if (prediction?.probability) {
            ...
            if (isNaN(pnrProbability))
                pnrProbability = undefined;
        }
        if (hasCNF && !hasWL) {
            pnrStatus = 'CONFIRMED';
...
// ─── Summary Builder ──────────────────────────────────────────────────────────
function buildSummary(candidates, ctx) {
...
class JourneyOrchestrator {
    ...
    orchestrate(context) {
        ...
        // Feature flag gate
        if (!featureFlags_1.featureFlags.journeyOrchestrator) {
        ...
        try {
            ...
            const directCandidates = normalizeDirectCandidates(context.searchResult ?? null, context.splitResult ?? null, context);
            const splitCandidates = normalizeSplitCandidates(context.splitResult ?? null, context);
            const rescueCandidates = normalizeRescueCandidates(context.rescueResult ?? null, context.splitResult ?? null, context);
            ...
            const health = assessHealth(context.liveResult ?? null, context.pnrResult ?? null, context.pnrPrediction ?? null);
            // 5. Build summary
            const summary = buildSummary(allCandidates, context);
            // 6. Pan India Rescue Intelligence Layer (PHASE_4C828)
            let intelligenceResult = null;
            if (featureFlags_1.featureFlags.panIndiaRescue) {
                intelligenceResult = rescueIntelligence_1.rescueIntelligence.evaluateRescue(context);
    ...
    disabledResult(context, evaluatedAt) {
        return {
            context,
            summary: {
                journeyType: 'NONE',
    ...
    errorResult(context, evaluatedAt, errorMsg) {
        return {
            context,
            summary: {
                journeyType: 'NONE',
...
exports.JourneyOrchestrator = JourneyOrchestrator;
// ─── Export ───────────────────────────────────────────────────────────────────
exports.journeyOrchestrator = new JourneyOrchestrator();
