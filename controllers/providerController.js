...
exports.deleteProvider = exports.updateProvider = exports.createProvider = exports.getProviders = exports.fallbackProviders = void 0;
...
const createProvider = async (req, res) => {
    try {
        ...
        const encryptedKey = providerConfigService_1.providerConfigService.encryptKey(api_key);
        try {
            ...
            logger_1.winstonLogger.warn(`[Admin] DB create provider RPC failed: ${dbError.message}. Using in-memory fallback.`);
            const newProvider = {
            ...
            providerConfigService_1.providerConfigService.flushCache(provider_name);
            logger_1.winstonLogger.info(`[api_provider_toggled] Telemetry logged. Provider toggled: ${provider_name}`);
...
const updateProvider = async (req, res) => {
    try {
        ...
        try {
            ...
            logger_1.winstonLogger.info(`[Admin] Invoking admin_update_provider_rpc for provider_id: ${id} with payload: ${JSON.stringify(loggedPayload)}`);
            const rpcResponse = await supabase_1.supabase.rpc('admin_update_provider_rpc', rpcPayload);
            const { data: rpcData, error } = rpcResponse;
            if (error) {
                logger_1.winstonLogger.error(`[Admin] RPC admin_update_provider_rpc failed for provider_id: ${id}. Error: ${error.message} (Code: ${error.code}). Response data: ${JSON.stringify(rpcData)}`);
                throw error;
            }
            logger_1.winstonLogger.info(`[Admin] RPC admin_update_provider_rpc succeeded for provider_id: ${id}. Response: ${JSON.stringify(rpcData)}`);
            ...
            providerConfigService_1.providerConfigService.flushCache(existing.provider_name);
            logger_1.winstonLogger.info(`[api_provider_toggled] Telemetry logged. Provider toggled: ${existing.provider_name}`);
