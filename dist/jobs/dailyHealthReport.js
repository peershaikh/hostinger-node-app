...
class DailyHealthReportJob {
    start() {
        logger_1.winstonLogger.info('[DAILY_HEALTH_REPORT] Scheduled for 08:00 IST (02:30 UTC) every day.');
    ...
    async generateAndSend() {
        ...
        try {
            ...
            logger_1.winstonLogger.warn(`[DAILY_HEALTH_REPORT] AI report unavailable: ${aiErr.message}`);
        }
        // In-process snapshots — zero I/O, never throw
        const eventStats = eventMetrics_1.eventMetrics.snapshot();
        const cacheStats = userCache_1.userCache.getStats();
        const availStats = smartAvailabilityMetrics_1.smartAvailabilityMetrics.getSnapshot();
        ...
        const adminEmail = adminEmailList.length === 1 ? adminEmailList[0] : adminEmailList;
        ...
        const aiFixes = digestPayload.ai_fixes.slice(0, 5)
            .map((f) => `<li><strong>${f.issue}</strong> — ${f.recommendation}</li>`).join('');
  ...
  <div style="background:white;padding:20px;border:1px solid #e5e7eb;">
    ...
    <p style="color:#6b7280;font-size:12px;margin-top:30px;border-top:1px solid #e5e7eb;padding-top:10px;">
      Generated automatically by Trayago Ops System · ${new Date().toISOString()}
        ...
        let digestStatus = 'SENT';
        let failureReason;
        let sentAt;
        try {
            await emailService_1.emailService.sendHealthReportEmail(adminEmail, subject, html);
            sentAt = new Date().toISOString();
            logger_1.winstonLogger.info(`[DAILY_HEALTH_REPORT] Digest sent to ${adminEmail}.`);
        }
        catch (emailErr) {
            digestStatus = 'FAILED';
            failureReason = emailErr.message;
            logger_1.winstonLogger.error(`[DAILY_HEALTH_REPORT] Email send failed: ${emailErr.message}`);
        }
        ...
        try {
            ...
            if (upsertError) {
                logger_1.winstonLogger.warn(`[DAILY_HEALTH_REPORT] Persist failed: ${upsertError.message}`);
            }
            else {
                logger_1.winstonLogger.info(`[DAILY_HEALTH_REPORT] Digest persisted for ${reportDate}.`);
            }
        }
        catch (persistErr) {
            logger_1.winstonLogger.warn(`[DAILY_HEALTH_REPORT] Persist exception: ${persistErr.message}`);
        }
    }
}
