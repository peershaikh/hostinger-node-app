...
class DbService {
    ...
    async dbLookupTrainName(trainNo) {
        ...
        if (!cleanNo || cleanNo === '00000' || cleanNo === 'N/A')
            return undefined;
        ...
        try {
            ...
            if (Array.isArray(dbTrains)) {
                const local = dbTrains.find((t) => lookupNumbers.includes(String(t.trainNo || t.number || '')));
        ...
        try {
            ...
            logger_1.winstonLogger.warn(`Train name DB lookup failed for ${trainNo}: ${e.message}`);
        }
        ...
        try {
            ...
            if (irctcService) {
                ...
                if (info) {
                    const liveName = info.trainInfo?.train_name ||
