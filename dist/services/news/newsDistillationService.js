...
exports.newsDistillationService = exports.NewsDistillationService = exports.NewsFactValidator = void 0;
const logger_1 = require("../../middleware/logger");
const aiProviderResolver_1 = require("../ai/aiProviderResolver");
class NewsFactValidator {
    static validate(source, output) {
        ...
        if (unsupportedEntities.length > 0) {
            logger_1.winstonLogger.warn('[NEWS_AI_VALIDATOR_REJECT] Hallucinated/unsupported entities detected in AI draft', {
...
class NewsDistillationService {
     ...
     */
    async distillArticle(article) {
        if (this.processedHashes.has(article.content_hash) && article.status === 'AI_DRAFTED') {
            logger_1.winstonLogger.info(`[NEWS_AI_SKIPPED_DUPLICATE] Article ${article.id} hash already processed.`);
        ...
        try {
            ...
            logger_1.winstonLogger.warn(`[NEWS_AI_DISTILL_ERROR] AI call failed for ${article.id}: ${err.message}`);
        }
        if (!aiOutput) {
            logger_1.winstonLogger.info(`[NEWS_AI_FALLBACK_APPLIED] Using deterministic extraction for ${article.id}`);
...
exports.NewsDistillationService = NewsDistillationService;
exports.newsDistillationService = new NewsDistillationService();
