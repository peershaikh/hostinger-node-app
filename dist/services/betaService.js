...
class BetaService {
            ...
            if ((0, supabase_1.isSupabaseConfigured)()) {
                supabase_1.supabase.from('beta_codes').insert({
                    ...
                    if (error && error.code !== '23505') {
                        logger_1.winstonLogger.warn(`[BETA_SERVICE] Failed to seed AZADI80 code to DB: ${error.message}`);
                    }
                });
            }
        }
    ...
    async upsertPromoCode(codeData) {
        ...
        if (existing) {
            ...
            if (codeData.targetPlan !== undefined)
                existing.targetPlan = codeData.targetPlan;
            if (codeData.durationDays !== undefined)
                existing.durationDays = codeData.durationDays;
