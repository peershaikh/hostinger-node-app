...
exports.railwayNewsService = {
    ...
    refreshNews: async () => {
        if (inFlightRefresh) {
            logger_1.winstonLogger.info('[NEWS_REFRESH_MUTEX] Refresh already in progress; reusing in-flight promise.');
            ...
            try {
                ...
                if (processedCanonical.length > 0 && (0, supabase_1.isSupabaseConfigured)()) {
                    try {
                        ...
                        const dedupedCanonical = processedCanonical.filter(a => seenUpsertIds.has(a.id) ? false : (seenUpsertIds.add(a.id), true));
