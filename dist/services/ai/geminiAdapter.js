...
exports.geminiAdapter = exports.GeminiAdapter = void 0;
...
class GeminiAdapter {
    constructor() {
        ...
        this.displayName = 'Google Gemini (Gemini 3.6 Flash)';
        this.capabilities = {
            predictPnr: true,
            analyzeRoute: true,
            enrichRoute: true,
    ...
    getApiKey() {
        return aiConfig_1.aiConfig.gemini.apiKey || process.env.GEMINI_API_KEY || '';
    }
    /**
    ...
    getActiveModel(featureKey) {
        ...
        return aiConfig_1.aiConfig.gemini.model || 'gemini-3.6-flash';
    }
    getEndpointUrl(featureKey) {
        const model = this.getActiveModel(featureKey);
        const baseUrl = aiConfig_1.aiConfig.gemini.baseUrl || 'https://generativelanguage.googleapis.com/v1beta/models';
     ...
     */
    async probeWithModel(prompt, model, json = true) {
        const apiKey = this.getApiKey();
        if (!apiKey) {
            throw new aiProvider_1.AiError({
        ...
        const finalPrompt = json
            ...
            : prompt;
        const timeout = aiConfig_1.aiConfig.gemini.timeoutMs || 10000;
        logger_1.winstonLogger.info(`[AI_PROBE] [GEMINI] Model: ${model} (isolated probe — no config mutation)`);
        ...
        if (!text)
            throw new aiProvider_1.AiError({ code: 'INVALID_RESPONSE', message: 'Probe returned empty response', provider: this.providerId });
        if (json) {
     ...
     */
    async executeGeminiCall(prompt, json = false, timeoutMs, featureName = 'GENERIC') {
        ...
        try {
            ...
            aiObservabilityService_1.aiObservabilityService.recordAiUsage({
                ...
                latencyMs,
                fallbackUsed: false,
                errorCategory: err?.code || (axios_1.default.isAxiosError(err) ? 'HTTP_ERROR' : 'EXECUTION_ERROR')
            });
            if (err instanceof aiProvider_1.AiError)
                throw err;
            if (axios_1.default.isAxiosError(err)) {
                ...
                if (status === 429) {
                    ...
                    const quotaReason = (errData?.details?.[0]?.violations?.[0]?.description ||
                        errData?.message ||
                        'Gemini API rate limit exceeded').replace(/[\r\n]+/g, ' ');
                    logger_1.winstonLogger.info(`[GEMINI_RATE_LIMIT_DETAILS] status=429 code=${errData?.code || 'RESOURCE_EXHAUSTED'} retry_after=${retryAfter || 'none'} reason="${quotaReason.slice(0, 150)}"`);
                    throw new aiProvider_1.AiError({
                        code: 'RATE_LIMITED',
    ...
    async categorizeFeedback(input) {
        const prompt = `
...
Return ONLY a JSON object with these exact keys:
{
  ...
  "suggestedAction": "One specific technical action for the dev team (max 15 words)"
    ...
    async generateSchedule(trainNo) {
...
- If unknown, return empty stations array.`;
    ...
    async normalizeAvailability(rawAvailString) {
...
Return ONLY a JSON array, e.g., [{"class": "3A", "status": "AVAILABLE", "count": 45}].`;
    ...
    async distillNewsArticle(input) {
...
Return ONLY valid JSON matching this schema:
{
  ...
  "confidence": "HIGH"
    ...
    async suggestAlternatives(source, destination) {
        const prompt = `
...
exports.geminiAdapter = new GeminiAdapter();
