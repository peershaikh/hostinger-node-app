...
function parseLegAvailability(rawAvail) {
    let text = '';
    let rawStatus = '';
    let providerPercentage = null;
    const availArr = extractAvailabilityArray(rawAvail);
    if (availArr && availArr.length > 0) {
    ...
    if (wlMatches && wlMatches[1]) {
        ...
        if (anyDigit)
            wlNumber = parseInt(anyDigit[0], 10);
    }
    ...
    async findSegmentSplits(source, destination, date, directTrains, classType = '3A', quota = 'GN') {
        ...
        for (const train of candidates) {
            ...
            for (const item of hubsWithStops) {
                ...
                if (segmentTier !== null) {
                    logger_1.winstonLogger.info(`[SEGMENT_ENGINE] 🎉 Same-train segment split (${segmentTier}) found on train ${trainNo} via ${hub}`);
                    ...
                    const availBadge = (() => {
                        if (segmentTier === 'CONFIRMED')
                            return 'CONFIRMED';
                        if (segmentTier === 'PARTIAL_RAC')
                            return 'RAC';
    ...
    async _buildHubsFromStops(stops, srcCode, destCode) {
        ...
        const intermediateStops = stops.filter(s => {
            ...
            return sn > srcSn && sn < destSn
                ...
                && !(0, stationAliases_2.areStationsCompatible)(s.Station_Code, srcStop.Station_Code)
                && !(0, stationAliases_2.areStationsCompatible)(s.Station_Code, destStop.Station_Code);
        });
        ...
        const hubPromises = hubsWithStops.map(async (item) => {
            ...
            if (hubTier !== null) {
                ...
                if (isConfirmed) {
                    ...
                    logger_1.winstonLogger.info(`[RESCUE_ENGINE] ⚠️ Partial RAC rescue option found via ${hub}`);
                }
                else {
                    logger_1.winstonLogger.info(`[RESCUE_ENGINE] ℹ️ High-chance WL rescue option found via ${hub} (minChance=${minChance}%)`);
                }
                return rescue;
            }
            else {
                ...
                logger_1.winstonLogger.info(`[RESCUE_ENGINE] Hub ${hub} rejected — leg1=${t1} leg2=${t2} (exceeds WL 50 or TQWL/unavailable)`);
                return null;
            }
        });
