...
exports.pnrController = exports.PnrController = void 0;
...
class PnrController {
    constructor() {
        this.getStatus = async (req, res) => {
            ...
            try {
                ...
                if (!this.isValidPnrResponse(rawStatus)) {
                    ...
                    try {
                        ...
                        if (cachedDbRecord) {
                            ...
                            logger_1.winstonLogger.info(`[PNR_SUCCESS] Database fallback found cached record for ${pnr}`);
                        }
                    }
                    catch (dbErr) {
                        logger_1.winstonLogger.warn(`[PNR_DB_FALLBACK_FAILED] ${pnr}: ${dbErr.message}`);
                    }
                }
                if (!this.isValidPnrResponse(rawStatus)) {
                    logger_1.winstonLogger.warn(`[PNR_FALLBACK] No live PNR available, issuing estimated response for ${pnr}`);
                ...
                logger_1.winstonLogger.info(`[PNR RAW RESPONSE] source=${pnrSource} pnr=${pnr}: ${JSON.stringify(rawStatus)}`);
                // Normalize the raw data for mapping using our shared helper
                const normalized = (0, pnrNormalizer_1.normalizeRawPnr)(rawStatus);
                ...
                if (journeyStr && journeyStr !== 'N/A') {
                    ...
                    if (journeyDate) {
                        ...
                        if (hoursSinceJourney >= 96) {
                            logger_1.winstonLogger.warn(`[PNR_EXPIRED] PNR ${pnr} has expired beyond allowed window (${hoursSinceJourney.toFixed(1)}h since journey). Returning fallback.`);
                            return res.status(200).json(this.buildPnrFallback(pnr));
                        }
                    }
                }
                // 2. Identify "Worst" Status for Prediction & Evaluate Passenger Mix
                let worstStatusType = 'CNF';
                let minWLPos = 999;
                let cnfCount = 0;
                let wlCount = 0;
                let racCount = 0;
                ...
                if (isEntitledToPrediction) {
                    ...
                    if (isChartPrepared) {
                        ...
                        try {
                            ...
                            const historicalData = await pnrHist.getHistoricalDataForPrediction(normalized.source_code || normalized.source_name || '', normalized.destination_code || normalized.destination_name || '', targetPassenger?.current_status || targetPassenger?.booking_status || '', pnr);
                        ...
                        try {
                            ...
                            if (historicalRate !== null) {
                                ...
                                if (rawProb > ceiling + CEILING_TOLERANCE) {
                                    ...
                                    logger_1.winstonLogger.warn(`[PNR_PREDICTION_CAP] AI gave ${rawProb}% for ${wlType}/${wlPosition} (ceiling=${ceiling}). Clamped to ${finalProb}%.`);
                                }
                                probability = String(finalProb);
                                predictionText = aiPrediction.prediction;
                                advice = aiPrediction.advice;
                                const activeProvider = (aiPrediction.providerId || '').toUpperCase();
                ...
                try {
                    const { learningService } = require('../services/learningService');
                    await learningService.logPnrCheck(pnr, cleanedPassengers[0]?.booking_status || 'Unknown', cleanedPassengers[0]?.current_status || 'Unknown', cleanResponse.data.chart_status !== 'Chart Not Prepared');
                    await learningService.trackApiUsage('pnr');
                    logger_1.winstonLogger.info(`[PNR_LEARNING] Logged learning data for PNR: ${pnr}`);
                }
                catch (saveError) {
                    logger_1.winstonLogger.error(`[PNR_LEARNING] Failed to log learning for PNR: ${pnr}: ${saveError.message}`);
                }
                try {
                    ...
                    logger_1.winstonLogger.info(`[PNR_HISTORY] Saved full journey history for PNR: ${pnr}`);
                }
                catch (histError) {
                    logger_1.winstonLogger.warn(`[PNR_HISTORY] Failed to save history for PNR: ${pnr}: ${histError.message}`);
                }
                // LOG PARSED OUTPUT
                ...
                logger_1.winstonLogger.error(`PNR Controller Error: ${err.message}`);
                return res.status(200).json(this.buildPnrFallback(pnr));
            }
        };
    }
    isValidPnrResponse(rawStatus) {
        if (!rawStatus || typeof rawStatus !== 'object')
            return false;
        if (rawStatus.success === false || rawStatus.status === false || rawStatus.error)
            return false;
        const pnrKeys = [
            'pnr',
            'PNR',
        ...
        for (const key of pnrKeys) {
            if (rawStatus[key] || rawStatus.data?.[key])
                return true;
        }
        if (Array.isArray(rawStatus.passengers) && rawStatus.passengers.length > 0)
            return true;
        if (Array.isArray(rawStatus.data?.passengers) && rawStatus.data.passengers.length > 0)
            return true;
        if (rawStatus.trainNumber || rawStatus.train?.number || rawStatus.data?.trainNumber || rawStatus.data?.train?.number)
            return true;
        return false;
    }
    buildPnrFallback(pnr) {
        logger_1.winstonLogger.warn(`[PNR_UNAVAILABLE] Both providers returned no data for PNR ${pnr}. Returning honest error.`);
        return {
            success: false,
    ...
    getConfidenceLabel(prob, statusType) {
        if (statusType === 'CNF')
            return 'Confirmed \u2013 Enjoy Your Journey';
        if (statusType === 'RAC')
            return 'Strong Chance \u2013 RAC Seat Confirmed';
        if (prob === null)
            return 'Prediction Unavailable';
        if (prob >= 75)
            return 'Strong Confirmation Chance';
        if (prob >= 50)
    ...
    async listTracked(req, res) {
        const { sessionId } = req.params;
        const list = await pnrTrackingService_1.pnrTrackingService.listBySession(sessionId);
        // Auto-clear flags when listing (user has seen them)
        await pnrTrackingService_1.pnrTrackingService.clearChangeFlag(sessionId);
    ...
    async upgrade(req, res) {
        ...
        const { pnr } = req.params;
        logger_1.winstonLogger.info(`[PNR_UPGRADE] Request for PNR: ${pnr} by user ${userId}`);
        const { error } = await require('../config/supabase').supabase
            .from('pnr_tracking')
            .update({ is_pro: true })
            .eq('pnr_number', pnr);
        if (error) {
            logger_1.winstonLogger.error(`[PNR_UPGRADE] Error: ${error.message}`);
            return res.status(500).json({ success: false, error: 'Database update failed' });
        }
        res.status(200).json({ success: true, message: 'Upgraded to PRO' });
    }
}
exports.PnrController = PnrController;
exports.pnrController = new PnrController();
